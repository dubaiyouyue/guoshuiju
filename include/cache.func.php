...
function cache_otherinfo($retype=1){
	global $db,$gz_otherinfo,$lang;
    $data = $db->get_one("SELECT * FROM $gz_otherinfo where lang='$lang' order by id");
	return cache_page('otherinfo_'.$lang.'.inc.php',$data,$retype);
	
}
function cache_str(){
	global $db,$gz_label,$lang;
	$query = "SELECT * FROM $gz_label where lang='$lang' order BY char_length(oldwords) DESC";
	$result = $db->query($query);
	while($list = $db->fetch_array($result)) {
		if(!$list[newwords]){
...
function cache_column(){
	global $db,$gz_column,$lang;//mobile
	$query="select * from $gz_column where display='0' and lang='$lang' order by classtype desc,no_order";
	$result= $db->query($query);
	while($list = $db->fetch_array($result)){
