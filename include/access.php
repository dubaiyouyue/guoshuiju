...
$navurl=($index=='index')?'':'../';
$gz_js_ac=" document.body.innerHTML=''; alert('".$lang_access."'); location.href='".$navurl."member/".$member_index_url."&referer='+encodeURIComponent(window.location.href);";
$query="select * from $gz_user_group where id='$metaccess'";
...
$gz_js_ac='document.write("'.authcode($listinfo, 'DECODE', $gz_member_force).'")';
}else{
	$gz_js_ac='document.write("'."【<a href='".$navurl."member/$member_login_url' target='_blank'>$lang_login</a>】【<a href='".$navurl."member/$member_register_url' target='_blank'>$lang_register</a>】".'")';
}
}else{
if(intval($metinfo_member_type)>=intval($metaccess)){
    $gz_js_ac="";
