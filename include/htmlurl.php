...
if(isset($murlid)){
	...
	if($list == 1) {
		if($htmlurl == 'message_list' || $htmlurl == 'index_list'){
			$query = "select * from {$gz_column} where foldername='message' and lang='{$lang}'";
			$url_column = $db->get_one($query);
			$metid = $url_column['id'];
			if($htmlurl == 'message_list'){
				$html_gz_htmlistname = 1;
			}else{
				$html_gz_htmlistname = 0;
			}
			...
			if(count($murlids) > 2){
				$html_gz_listhtmltype = 0;
			}
		}else{
			$metid = $htmlurl;
			if(!$lang){
				$url_column =array();
				$query = "select * from {$gz_column} where filename='{$htmlurl}'";
				$url_column = $db->get_one($query);
				if($url_column){
		...
		if(preg_match('/^(showproduct|shownews|showimg|showdownload|about|job|[0-9]{8}|'.$furlid.')[0-9]+/', $htmlurl, $out)){
			...
			if($fmodule != 1){
				if($out[1] == $furlid){
					$html_gz_htmpagename = 2;
				}else if(is_numeric($out[1])){
					$html_gz_htmpagename = 1;
				}else{
					$html_gz_htmpagename = 0;
				}
			}
		}else{
			$metid = $htmlurl;
			if(!$lang){
				$url_column = array();
				$table = array();
				$query = "select * from {$gz_column} where foldername='{$furlid}'";
				$url_column = $db->get_one($query);
				$table[1] = $gz_column;
				$table[2] = $gz_news;
				$table[3] = $gz_product;
