...
global $gz_weburl,$gz_host,$gz_file;
$host=$gz_host;
$file=$gz_file;
	if(get_extension_funcs('curl')&&function_exists('curl_init')&&function_exists('curl_setopt')&&function_exists('curl_exec')&&function_exists('curl_close')){
		$curlHandle=curl_init(); 
		curl_setopt($curlHandle,CURLOPT_URL,'http://'.$host.$file); 
		curl_setopt($curlHandle,CURLOPT_REFERER,$gz_weburl);
...
function varcodeb($type){
	global $gz_file,$db,$gz_otherinfo;
	$blcode = $db->get_one("SELECT * FROM $gz_otherinfo where id='1'");
	$authcode = $blcode['authcode'];
	$authpass = $blcode['authpass'];
	$gz_file='/test/varcode.php';
...
function sendsms($phone,$message,$type){
	global $db,$gz_otherinfo,$gz_sms,$gz_file;
...
function maxnurse(){
	...
	$maxnurse = $db->get_all("SELECT * FROM $gz_sms WHERE time>='{$ct}' and time<='{$et}' and type='4' and remark='SUCCESS'");
