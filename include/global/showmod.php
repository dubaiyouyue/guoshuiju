...
if(!$pagemark){
	if($dbname==$gz_news)$pagemark = 2;
	if($dbname==$gz_product)$pagemark = 3;
	if($dbname==$gz_download)$pagemark = 4;
	if($dbname==$gz_img)$pagemark = 5;	
}
$acc_sql = '';
$news['updatetime_order']=$news['updatetime'];
$news['updatetime'] = date($gz_contenttime,strtotime($news['updatetime']));
if(strstr($news['imgurls'], "http://")){
	$news['imgurls']=($news['imgurls']<>"")?$news['imgurls']:$news['imgurls'];
}else{	
	$news['imgurls']=($news['imgurls']<>"")?$news['imgurls']:'../'.$gz_agents_img;
	}
if(strstr($news['imgurl'], "http://")){
	$news['imgurl']=($news['imgurl']<>"")?$news['imgurl']:$news['imgurl'];
}else{	
	$news['imgurl']=($news['imgurl']<>"")?$news['imgurl']:'../'.$gz_agents_img;
}
$class1=$news['class1'];
$class2=$news['class2'];
$class3=$news['class3'];	
...
if($imgproduct=='download'){
	if(intval($news['downloadaccess'])>0&&$gz_member_use){
...
if($mdmendy){
	$query1 = "select * from $gz_plist where module='$pagemark' and listid='$id'";
	...
	while($list1 = $db->fetch_array($result1)){
		...
		if($metpara[$list1['paraid']]['type']==5){
			...
			$news[$nowpara1]="<a href='{$list1['info']}' {$metblank} class='gz_annex {$fltp}' title='{$list1['imgname']}'>{$list1['imgname']}</a>";
			$news[$nowpara1.'s']=$list1['info'];
		}
		if(intval($metparaaccess)>0&&$gz_member_use){
			$paracode=authcode($news[$nowpara1], 'ENCODE', $gz_member_force);
...
if($dataoptimize[$pagemark]['nextlist']){
	...
	$acc_sql=$gz_member_use==2?"(access<='$metinfo_member_type') and":" ";
	...
	//上下篇划分
	if($class1){$csql="class1='$class1'";}
	if($class2){$csql="class2='$class2'";}
	if($class3){$csql="class3='$class3'";}
...
if($dataoptimize[$pagemark]['otherlist']){	
	...
	while($list= $db->fetch_array($result)){
		...
		$list['hot']=$list['top_ok']?"":(($list['hits']>=$gz_hot)?"<img class='listhot' src='".$img_url."hot.gif"."' />":"");
		$list['news']=$list['top_ok']?"":((((strtotime($m_now_date)-strtotime($list['updatetime']))/86400)<$gz_newsdays)?"<img class='listnews' src='".$img_url."news.gif"."' />":"");
		...
		if(strstr($list['imgurls'], "http://")){
			...
			$list['imgurls']=($list['imgurls']<>"")?$list['imgurls']:'../'.$gz_agents_img;
		}
		if(strstr($list['imgurl'], "http://")){
			$list['imgurl']=($list['imgurl']<>"")?$list['imgurl']:$list['imgurl'];
		}else{	
			$list['imgurl']=($list['imgurl']<>"")?$list['imgurl']:'../'.$gz_agents_img;
		}
		if($dataoptimize[$pagemark]['para'][$pagemark]){
			$query1 = "select * from $gz_plist where module='$pagemark' and listid='$list[id]'";
			$result1 = $db->query($query1);
			while($list1 = $db->fetch_array($result1)){
				$nowpara1="para".$list1['paraid'];
...
if($dataoptimize[$pagemark]['nextlist']){
	...
	$gz_ahtmtypep = $prenews['filename']<>''?$gz_chtmtype:$gz_htmtype;
	$gz_ahtmtypen = $nextnews['filename']<>''?$gz_chtmtype:$gz_htmtype;
	if($prenews)$prenews['url']=$gz_pseudo?$preid.'-'.$lang.'.html':($gz_webhtm?($prenews['filename']?$preid.$gz_ahtmtypep:$prehtmname.$prenews['id'].$gz_ahtmtypep):$phpname.$prenews['id']);
    if($nextnews)$nextnews['url']=$gz_pseudo?$nextid.'-'.$lang.'.html':($gz_webhtm?($nextnews['filename']?$nextid.$gz_ahtmtypep:$nexthtmname.$nextnews['id'].$gz_ahtmtypen):$phpname.$nextnews['id']);
	$preinfo=$prenews;
	$nextinfo=$nextnews;
...
$show['description']= $news['description']?$news['description']:$gz_description;
$show['keywords']   = $news['keywords']?$news['keywords']:$gz_keywords;
$gz_title          = $news['ctitle']?$news['ctitle']:($gz_title?$news['title'].'-'.$gz_title:$news['title']);
...
if($news[tag]){
	...
	foreach($tags as $key=>$val){
		...
		if($gz_pseudo||$gz_tag_pseudo){$tagstr.="&nbsp<a href=\"../tag/{$urlval}-{$lang}\" target=\"_blank\">$val</a>";}
...
if($metinfonow==$gz_member_force and $gz_webhtm){
	$html_filenamex=str_replace("\\",'',$html_filename);
	$html_filenamex=unescape($html_filenamex);
	$news['url']=$gz_weburl.$class_list[$class1]['foldername'].'/'.$html_filenamex.$gz_htmtype;
}
