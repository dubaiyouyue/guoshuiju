...
if($dbname!=$gz_download&&$dbname!=$gz_img&&$dbname!=$gz_news&&$dbname!=$gz_product){okinfo('../404.html');exit();}
if($class_list[$class1]['module']>=100||($class1==0&&$class2==0&&$class3==0)||$class1==10001){
	if($search=="search"){
		...
		$query="select * from $gz_column where module='$search_module' and (classtype=1 or releclass!=0) and lang='$lang' order by no_order ASC,id ASC";
...
if($gz_member_use){
	...
	$classaccess= $db->get_one("SELECT * FROM $gz_column WHERE id='$classaccess'");
	$metaccess=$classaccess['access'];
}
require_once '../include/head.php';
if($class1){if(!is_array($class_list[$class1]))okinfo('../404.html');}
$pseudos=$db->get_one("select * from $gz_column where filename='$class2' and lang='$lang'");
if($pseudos){
$class2=$pseudos[id];
}
if($class2){
...
if($search=="search" && $mdmendy){ 
	...
	if($searchtype){
		...
		foreach($dbparaname as $key=>$val){
			...
			if(trim($paratitle)<>''){
				$serch_sql .= " and exists(select * from $gz_plist where module=3 and $gz_plist.paraid='$val[id]' and $gz_plist.listid='$dbname.id and' $gz_plist.info='".trim($paratitle)."') "; 
		...
		foreach($dbparaname as $key=>$val){
			...
			if($val['type']==4){
				if(!$paratitle){
					...
					if(trim($paratitle)<>'')$paratitle=substr($paratitle, 0, -1);
					if(trim($paratitle)<>''){
						$serch_sql .= " and exists(select * from $gz_plist where module=3  and $gz_plist.paraid='$val[id]' and $gz_plist.listid=$dbname.id and $gz_plist.info like'%".trim($paratitle)."%') ";  
						$serchpage .= "&".$val['para']."=".trim($paratitle);
					}
				}else{
					$serch_sql .= " and exists(select * from $gz_plist where module=3 and $gz_plist.paraid='$val[id]' and $gz_plist.listid=$dbname.id and $gz_plist.info like'%".trim($paratitle)."%') ";  
		...
		//5.0.4
		if($content<>'')$serch_sql .= " or exists(select $gz_plist.id from $gz_plist inner join $gz_parameter on $gz_plist.paraid=$gz_parameter.id where $gz_plist.module=3 and $gz_parameter.type<>5 and $gz_plist.listid=$dbname.id and $gz_plist.info like'%".trim($content)."%')) ";
		//价格搜索
		foreach($dbparaname as $key=>$val2){
			...
			if($prices){
					if(!strstr($prices, "-")){
						...
						$results=$result[0];
						$serch_sql .= " and exists(select * from $gz_plist where module=3 and $gz_plist.paraid='$val2[id]' and $gz_plist.listid=$dbname.id and $gz_plist.info > $results) "; 
						...
						$serch_sql .= " and exists(select * from $gz_plist where module=3 and $gz_plist.paraid='$val2[id]' and $gz_plist.listid=$dbname.id and $gz_plist.info > $prices_sql[0] and $gz_plist.info < $results) "; 
						$serchpage .= "&".$prices1."=".trim($$prices1);
					}
...
require_once '../include/pager.class.php';
	...
	if($class1_info['listnumber']>0){
		$list_num=$class1_info['listnumber'];
	}
	if($class2_info['listnumber']>0){
		$list_num=$class2_info['listnumber'];
	}
	if($class3_info['listnumber']>0){
		$list_num=$class3_info['listnumber'];
	}
	if(!$list_num){
	...
	foreach($modlistnow as $key=>$list){
		if($mdmendy){
			...
			if($dataoptimize[$pkgem]['para'][$pkgem]){
				$query1 = "select * from $gz_plist where listid='$list[id]' and module='$pkgem' ";
		...
		$list['top']=$list['top_ok']?"<img class='listtop' src='".$img_url."top.gif"."' alt='".$gz_alt."' />":"";
		$list['hot']=$list['top_ok']?"":(($list['hits']>=$gz_hot)?"<img class='listhot' src='".$img_url."hot.gif"."' alt='".$gz_alt."' />":"");
		$list['news']=$list['top_ok']?"":((((strtotime($m_now_date)-strtotime($list['updatetime']))/86400)<$gz_newsdays)?"<img class='listnews' src='".$img_url."news.gif"."' alt='".$gz_alt."' />":"");
		...
		if($list['links']){
			...
			$list['url']=$gz_pseudo?$panyid.'-'.$lang.'.html':($gz_webhtm?$htmname.$gz_ahtmtype:$phpname);
			if($class_list[$class1]['module']>=100||$search=='search'||$list['class1']!=$class1)$list['url']='../'.$class_list[$list['class1']]['foldername'].'/'.$list['url'];
		}
		if($mdname=='download'){
			if(intval($list['downloadaccess'])>0&&$gz_member_use){
...
if($search=='search' && $mdmendy){
	...
	if($gz_pseudo){
		...
		if($class3<>0){
			$htmclass=$gz_listhtmltype?'_'.$class3.'_':'_'.$class1.'_'.$class2.'_'.$class3.'_';
			$gz_pagelist=(($metadmin['pagename'] and $class_list[$class3]['filename']<>"")?$class_list[$class3]['filename']:($gz_htmlistname?$class_list[$class3]['foldername']:$modulename[$class_list[$class3]['module']][0])).$htmclass;
			$gz_pagelist=$class_list[$class3]['filename']<>''?$class_list[$class3]['filename'].'_':$gz_pagelist;
			$gz_ahtmtype = $class_list[$class3]['filename']<>''?$gz_chtmtype:$gz_htmtype;
		}elseif($class2<>0){
			$htmclass=$gz_listhtmltype?'_'.$class2.'_':'_'.$class1.'_'.$class2.'_';
			$gz_pagelist=(($metadmin['pagename'] and $class_list[$class2]['filename']<>"")?$class_list[$class2]['filename']:($gz_htmlistname?$class_list[$class2]['foldername']:$modulename[$class_list[$class2]['module']][0])).$htmclass;
			$gz_pagelist=$class_list[$class2]['filename']<>''?$class_list[$class2]['filename'].'_':$gz_pagelist;
			$gz_ahtmtype = $class_list[$class2]['filename']<>''?$gz_chtmtype:$gz_htmtype;
		}else{
			$gz_pagelist=(($metadmin['pagename'] and $class_list[$class1]['filename']<>"")?$class_list[$class1]['filename']:($gz_htmlistname?$class_list[$class1]['foldername']:$modulename[$class_list[$class1]['module']][0]))."_".$class1."_";
...
if($mdmendy){
	$dbpagename = $mdname=='product'?$gz_product_page:($mdname=='download'?$gz_download_page:$gz_img_page);
...
if(($gz_product_page && $class_list[$class1]['module']==3) || ($class_list[$class1]['module']==5 && $gz_img_page) && $search<>'search' && $metinfover=='v1'){
	$product_listp=array();
	if($class2 && count($nav_list3[$class2]) && !$class3){
	...
	if($product_listp){
		foreach($product_listp as $key=>$val){
			$val['title']=$val['name'];
			$val['imgurl']=$val['columnimg']==''?$weburly.$gz_agents_img:$val['columnimg'];
