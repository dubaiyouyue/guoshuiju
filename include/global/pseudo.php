...
$navurl    = $index=='index'?'':'../';
$gz_logox = explode('../',$gz_logo);
$weburly   = $index=='index'?'':'../';
if(!strstr($gz_logo, "http://")){
	$gz_logo  = $gz_pseudo?$weburly.$gz_logox[1]:($index=='index'?$gz_logox[1]:$gz_logo);
}
$skinurl   = 'templates/'.$gz_skin_user;
$css_url   = $weburly.$skinurl.'/css/';
$img_url   = $weburly.$skinurl.'/images/';
$gz_url   = $weburly.'public/';
$metweburl = ROOTPATH;
$weburly   = $index=='index'?'':'../';



$countlang = count($gz_langok);
if($gz_index_type==$lang)$countlang=1;
$dataoptimize_html='';
require_once file_exists($navurl.$skinurl.'/metinfo.inc.php')?$navurl.$skinurl.'/metinfo.inc.php':ROOTPATH.'config/metinfo.inc.php';
$metadmin[pagename]=1;
$cache_column=gz_cache(ROOTPATH.'cache/'."column_".$lang.".inc.php");
if(!$cache_column){
...
/*url地址*/
	...
	if($listc['filename'] && $listc['filename']!=''){
		$gz_ahtmtype=$gz_chtmtype;
	}else{
		$gz_ahtmtype=$gz_htmtype;
	}
		...
		switch($listc['module']){
			default:
				if($gz_pseudo){
					...
					$pudo_type= !$gz_htmlistname?$modulename[$listc['module']][0]:$listc['foldername'];
					if($listc['filename']<>"" and $metadmin['pagename']){
						$listc['url']=$urltop.$listc['filename'].'_1'.$gz_ahtmtype;
					}
					else{
						$psid=$pudo_type;
						if($gz_listhtmltype==0&&($listc['classtype']==2||$listc['classtype']==3)&&!$listc['releclass']){
							if($listc['classtype']==2){
								$listc['url']=$urltop.$psid.'_'.$listc['bigclass'].'_'.$listc['id'].'_1'.$gz_ahtmtype;
							}
							else{
								if($cache_column[$listc['bigclass']]['releclass']){
									...
									$listc['url']=$urltop.$psid.'_'.$cache_column[$listc['bigclass']]['bigclass'].'_'.$listc['bigclass'].'_'.$listc['id'].'_1'.$gz_ahtmtype;
								}
							}								
						}
						else{
							$listc['url']=$urltop.$psid.'_'.$listc['id'].'_1'.$gz_ahtmtype;
						}
					}
				}
				else{
			...
			case 1:
				if($listc['isshow']!=0){
					if($gz_pseudo){
						...
						$listc['url']=$urltop.$psid.$gz_ahtmtype;
					}
					else{
			...
			case 6:
				if($gz_pseudo){
					...
					$listc['url']=$urltop.$psid.'_1'.$gz_ahtmtype;
				}
				else{
			...
			case 7:
				if($gz_pseudo){
					...
					$pudo_type= !$gz_htmlistname?"index_list_1":"message_list_1";
			...
			case 10:
			case 12:
				$listc['url']=(($gz_pseudo)?$urltop.'index-'.$lang.'.html':(($gz_webhtm==2)?$urltop.'index'.$gz_ahtmtype:$urltop.'index.php?'.$langmark));
				break;	
			case 11:
				$listc['url']=($gz_pseudo)?$urltop.'index-'.$lang.'.html':$urltop.'index.php?'.$langmark;
				break;
		}
	}	
...
foreach($cache_column as $key=>$list){
	//if($list['display']==0){	
		$column_moblie_ok=1;
		if($gz_mobileok&&$gz_wap&&$gz_wapshowtype){
			if($list[wap_ok]){
				$column_moblie_ok=1;
...
$addmessage_url=$gz_pseudo?$navurl.'message/message-'.$lang.'.html':($gz_webhtm?$navurl.'message/message'.$gz_htmtype:$navurl.'message/message.php?'.$langmark);
$cv['url']=$gz_pseudo?'jobcv-0-'.$lang.'.html':($gz_webhtm?$navurl."job/cv".$gz_htmtype:$navurl."job/cv.php?".$langmark);
$addfeedback_url=$gz_pseudo?$navurl.'feedback/index-'.$lang.'.html':($gz_webhtm?$navurl.'feedback/'.$addfeedback_url.$gz_htmtype:$navurl.'feedback/index.php?'.$langmark);
$cv['url']=$gz_pseudo?'jobcv-0-'.$lang.'.html':$navurl."job/cv.php?".$langmark."&selectedjob=";
