...
function metstat(){
	var url=encodeURIComponent(window.location.href),lurl=encodeURIComponent(window.location.href),cookm={$kname}.statcookie('recordurl'),myDate = new Date();
...
if($type=='submit' && $gz_stat){
		...
		if((checkadd(1,$ip) && checkadd(2,$url) && $browser!='')){
			if(!$lurl || checkadd(2,$lurl)||$google_type=="ok"){
				...
				$lang=$d[2];
				$dizhi='';
				$dayquery ="INSERT INTO {$gz_visit_day} SET
							ip           = '{$ip}',
							acctime      = '{$stime}',
							visitpage    = '{$url}',
							antepage     = '{$lurl}',
				...
				$visit=$db->get_one("SELECT * FROM {$gz_visit_summary} WHERE stattime='{$dtime}'");
				$ztian0=strtotime(date("Y/m/d 00:00:00"));
				$ztian1=strtotime(date("Y/m/d 23:59:59"));
				$ipset=$ip;
				$gz_stat_maxok=1;
				if($visit){
					$vsip=$db->get_one("SELECT * FROM {$gz_visit_day} WHERE ip='{$ip}' and acctime >='{$ztian0}' and acctime<='{$ztian1}'"); 
					$pv = $visit['pv']+1;
					if($pv<$gz_stat_max){
						...
						$query = "update {$gz_visit_summary} SET
									pv         = '{$pv}',
									ip         = '{$ip}',
									alone      = '{$alone}',
									parttime   = '{$parttime}'
					...
					$query = "INSERT INTO {$gz_visit_summary} SET
								pv         = '1',
								ip         = '1',
								alone      = '1',
								parttime   = '{$parttime}',
				...
				if($gz_stat_maxok){
					...
					if(($lurlkey && delete($lurlkey[0])!='') || $lurlkeyk=='无法获取'){
						...
						$keyok=$db->get_one("SELECT * FROM {$gz_visit_detail} WHERE name='{$lurlkey[0]}' and stattime = '{$dtime}' and type='1'");
						if($keyok){
							...
							$vsip=$db->get_one("SELECT * FROM {$gz_visit_day} WHERE ip='{$ipset}' and acctime >='{$ztian0}' and acctime<='{$ztian1}' and antepage = '{$lurl}'"); 
							$pv = $keyok['pv']+1;
							$ip = !$vsip?$keyok['ip']+1:$keyok['ip'];
							$alone = $vsip&&$cook?$keyok['alone']:$keyok['alone']+1;
							$query = "update {$gz_visit_detail} SET
										pv       = '{$pv}',
										ip       = '{$ip}',
										alone    = '{$alone}',
										remark   = '{$remark}',
							...
							if($lurlkey[0]!='' || $lurlkeyk=='无法获取'){						
									...
									$query = "INSERT INTO {$gz_visit_detail} SET
										name     = '{$lurlkey[0]}',
										pv       = '1',
										ip       = '1',
										alone    = '1',
					...
					$urlok=$db->get_one("SELECT * FROM {$gz_visit_detail} WHERE name='{$url}' and stattime = '{$dtime}' and type='2'");
					if($urlok){
						$vsip=$db->get_one("SELECT * FROM {$gz_visit_day} WHERE ip='{$ipset}' and acctime >='{$ztian0}' and acctime<='{$ztian1}' and visitpage ='{$url}'"); 
						$pv = $urlok['pv']+1;
					...
					$lurlok=$db->get_one("SELECT * FROM {$gz_visit_detail} WHERE name='{$lurl}' and stattime = '{$dtime}' and type='3'");
					if($lurlok){
						$vsip=$db->get_one("SELECT * FROM {$gz_visit_day} WHERE ip='{$ipset}' and acctime >='{$ztian0}' and acctime<='{$ztian1}' and antepage ='{$lurl}'"); 
						$pv = $lurlok['pv']+1;
