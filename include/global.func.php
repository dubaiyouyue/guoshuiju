...
global $gz_sqlinsert,$id,$class1,$class2,$class3;
	...
	if(is_array($string)){
		...
		*/
		if(strlen($string_old)!=strlen($string)&&$gz_sqlinsert){
			$reurl="http://".$_SERVER["HTTP_HOST"];
			echo("<script type='text/javascript'> alert('Submitted information is not legal!'); location.href='$reurl'; </script>");
			die("Parameter Error！");
		}
...
function template($template,$EXT="html"){
	global $gz_skin_user,$skin,$dataoptimize_html,$gz_mobileok,$metinfover;
...
function htmpacks($murl,$adminfile){
	global $classnow;
		$gz_htmpack_url=$adminfile.'/databack/htmpack/';
		$dir=getcwd();
		$dir=basename($dir);
		$pack_url = $gz_htmpack_url;
		$murlb=$murl;
		$murl = $pack_url.$murl;
...
/*页面输出*/
function footer(){	
	global $output,$db,$gz_htmtype,$html_filename,$metinfonow,$gz_member_force,$gz_webhtm,$htmpack,$lang_htmcreate,$lang_htmsuccess,$index_url,$indexy,$gz_chtmtype,$adminfile;
	...
	if($metinfonow==$gz_member_force and $gz_webhtm){
		...
		$html_filename.=$indexy=='index'?$gz_htmtype:($indexy==1?$gz_chtmtype:$gz_htmtype);
...
if($theme_preview){
var_export("-->");
echo "<script>alert(\"{$gz_weburl}未授权使用此模板或已经过期!Powered by MetInfo\");window.parent.close();</script>";
/*
...
*/
//okinfo("http://www.resonance.com.cn","{$gz_weburl}未授权使用此模板或已经过期! Powered by MetInfo");exit();
}
}
/*把字符串当成代码运行*/
...
/*手机跳转*/
function wapjump($tp){
	global $gz_wap_tpa,$gz_wap_tpb,$gz_wap_url,$gz_wap,$gz_mobileok,$lang;
...
function gz_cooike_start(){
	global $gz_cookie,$db,$gz_webkeys,$gz_admin_table,$lang,$gz_user;
	$gz_cookie=array();
	if($_COOKIE[acc_auth] && $_COOKIE[acc_key]){
		define('IN_MET', true);
		list($username,$password)=explode("\t",authcode($_COOKIE[acc_auth],'DECODE', $gz_webkeys.$_COOKIE[acc_key]));
		$username=daddslashes($username,0,1);
		$query="select * from $gz_user where username='$username'";
	...
	if($_COOKIE[gz_auth] && $_COOKIE[gz_key]){
		...
		if(md5($user['admin_pass'])==$password&&time()-$usercooike->time<3600){
			...
			$gz_cookie['time']=time();
			$gz_cookie['metinfo_member_type'] = 256;
...
function login_gz_cookie($userid){
	...
	$db->query("update $gz_admin_table set cookie='$json' WHERE admin_id='$userid'");
	$query="select * from $gz_admin_table WHERE admin_id='$userid'";
	$user=$db->get_one($query);
	$gz_key=gz_rand(7);
...
function jump_pseudo(){
	global $db,$gz_skin_user,$pseudo_jump;
	global $gz_column,$gz_news,$gz_product,$gz_download,$gz_img,$gz_job;
	global $class1,$class2,$class3,$id,$lang,$page,$selectedjob;
	global $gz_index_type,$index,$gz_pseudo;
	if($gz_pseudo){
		$metadmin[pagename]=1;
		...
		if($pseudo_jump!=1){
			...
			switch($dir_filenames[0]){
					...
					if(!$class1&&!$class2&&!$class3){
						if($index=='index'){
							...
							if($lang==$gz_index_type){
								...
								if($id){
									$query="select * from $gz_column where id='$id'";
								}else{
									$query="select * from $gz_column where foldername='$dir_dirname' and lang='$lang' and (classtype='1' or releclass!='0') order by id asc";
								}
						...
						if($dir_dirname=='job.php'){
							$query="select * from $gz_column where lang='$lang' and module='6'";
						}else{
							$id=$class3?$class3:($class2?$class2:$class1);
							$query="select * from $gz_column where id='$id'";
						}
				...
				case 'showjob.php':
					switch($dir_filenames[0]){
						case 'shownews.php':
						$query="select * from $gz_news where id='$id'";
						break;
						case 'showproduct.php':
						$query="select * from $gz_product where id='$id'";
						break;
						case 'showdownload.php':
						$query="select * from $gz_download where id='$id'";
						break;
						case 'showimg.php':	
						$query="select * from $gz_img where id='$id'";
						break;
						case 'showjob.php':
						$query="select * from $gz_job where id='$id'";
						break;
					}
					$jump=$db->get_one($query);
