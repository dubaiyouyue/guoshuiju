...
function link($url, $exc='')
  {
      global $lang_PageTotal,$lang_Page,$lang_PageLocation,$lang_PageHome,$lang_PageEnd,$lang_PagePre,$lang_PageNext,$lang_PageGo,$gz_pageskin,$total_count,$metinfover;
	  global $lang_Total,$lang_Pagenum,$gz_url,$langnums,$metinfouiok,$class1,$class2,$class3,$class_list,$classnow,$gz_pseudo,$search;
		if(!$exc&&($class_list[$classnow][module]<6)){
			$url=str_replace('&class2=0','',$url);
			$url=str_replace('&class3=0','',$url);
		}
 if($gz_pageskin=='' or $gz_pageskin==0 or $gz_pageskin>9)$gz_pageskin=1;
 ...
 $prepage=$langnums==1?'../'.$this->SELF.'/':$url.($this->_cur_page-1).$exc;
  ...
  if($metinfover == 'v1'){
    ...
    if ($this->_cur_page == 1){     //$this->_cur_page当前页面的码数
			if($this->pages!=0){
			 $text.="<a class='PreSpan'>$lang_PagePre</a>";
			}
		}else{
	...
	for($i=$startnum;$i<=$endnum;$i++){
	   ...
	   if($i==$this->_cur_page){$page_stylenow="class='Ahover'";}
	    if(!$page_stylenow) $text.="<a href=".$pageurl." $page_stylenow>".$i."</a>";
	    else $text.="<span href=".$pageurl." $page_stylenow>".$i."</span>";
		$page_stylenow='';
	   }
	   $text.=$lastPage;
	 if ($this->_cur_page == $this->pages){
        $text.="<a class='NextSpan'>$lang_PageNext</a>";
	  }else{
		if($this->pages!=0){
...
case 1:
  ...
  if(!$metinfouiok){
	  if($gz_pageskin==4){
	   ...
	   $text.=".digg4 a,.digg4 span.miy{ border:1px solid  #ff9600; padding:2px 7px 2px 7px; background-position:50% bottom; margin:2px; color:#ff6500; background-image:url(".$gz_url."images/page6.jpg); text-decoration:none;}";
	   ...
	   $text.=".digg4  a,.digg4 span.miy{ border:1px solid  #2c2c2c; padding:2px 5px 2px 5px; background:url(".$gz_url."images/page7.gif) #2c2c2c; margin:2px; color:#fff; text-decoration:none;}";
	   $text.=".digg4  a:hover { border:1px solid #aad83e; color:#fff;background:url(".$gz_url."images/page7_2.gif) #aad83e;}";
	   $text.=".digg4  a:active { border:1px solid #aad83e; color:#fff;background:urlurl(".$gz_url."images/page7_2.gif) #aad83e;}";
	   $text.=".digg4  span.current {border:1px solid #aad83e; padding:2px 5px 2px 5px; margin:2px; color:#fff;background:url(".$gz_url."images/page7_2.gif) #aad83e; text-decoration:none;}";
	   $text.=".digg4  span.disabled { border:1px solid #f3f3f3; padding:2px 5px 2px 5px; margin:2px; color:#ccc;}";
	   $text.=".digg4 .disabledfy { font-family: Tahoma, Verdana;}"; 
	   $text.=" </style>";  
	  }elseif($gz_pageskin==8){
	   $text.="<style>";
   ...
   if($this->pages >12 ){
	 ...
	 if(floor(($this->_cur_page/10))==($this->_cur_page/10))$startnum=floor(($this->_cur_page/10))*10+1-10;
	  ...
	  $text.="<div class='digg4 metpager_{$gz_pageskin}'>";
    if ($this->_cur_page == 1){
