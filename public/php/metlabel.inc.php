...
function metlabel_html5($closure=1,$iehack=1,$mobile=0){
	global $gz_title,$show,$m_now_year,$navurl,$gz_js_access,$gz_skin_css,$img_url,$gz_webname,$metcms_v,$appscriptcss,$gz_ch_lang,$lang,$gz_ch_mark,$gz_url,$metinfouiok,$classnow,$class_list,$gz_headstat,$metinfover,$id;
	global $gz_wap,$gz_wap_tpa,$gz_wap_tpb,$gz_webhtm,$gz_wap_url,$module,$metinfonow,$gz_member_force,$gz_weburl,$gz_wapshowtype;
	global $_M;
	//静态页面手机版跳转
	$mobileto = '';
	if($metinfonow==$gz_member_force&&$gz_webhtm&&$gz_wap&&($gz_wap_tpa||$gz_wap_tpb)&&isset($gz_wapshowtype)){
		$mobile_prefix=request_uri();
		$allidlist=explode('&metmemberforce=',$mobile_prefix);
		$mobile_prefix=$allidlist[0];
		if($gz_wap_tpb&&$gz_wap_url)$mobile_prefix=str_replace($gz_weburl,$gz_wap_url,$mobile_prefix);
		$mobileto.="<script type=\"text/javascript\">var gz_wap_tpa={$gz_wap_tpa},gz_wap_tpb={$gz_wap_tpb},gz_wap_url='{$gz_wap_url}',mobile_lang='{$lang}',mobile_prefix='{$mobile_prefix}';</script>\n";
	...
	if($mobile){
		...
		if($gz_js_access)$metinfo.=$gz_js_access."\n";
		...
		$metinfo.="<link rel=\"stylesheet\" type=\"text/css\" href=\"".$img_url."css/".$gz_skin_css."\" />\n";
		$metinfo.="<script src=\"{$navurl}public/js/jQuery1.7.2.js\" type=\"text/javascript\"></script>\n";
		if($gz_ch_lang and $lang==$gz_ch_mark)$metinfo.="<script src=\"".$gz_url."js/ch.js\" type=\"text/javascript\"></script>\n";
...
function metlabel_flash(){
	global $methtml_flash,$gz_flasharray,$classnow,$gz_flashimg,$navurl,$metinfover;
	if($gz_flasharray[$classnow][type]){
		if($gz_flasharray[$classnow][type]==1){
		if($metinfover=='v1'){
		switch($gz_flasharray[$classnow][imgtype]){
			case 6:

					$metinfo.="<div class='flash flash6' style=' height:".$gz_flasharray[$classnow][y]."px;'>
					";
					$metinfo.="
		<ul id='slider6' class='list-none'>
					...
					foreach($gz_flashimg as $key=>$val){
						...
						$metinfo.="
			<li>
				<a href='".$val[img_link]."' target='_blank' title='{$val[img_title]}' class='b6' style='background-image:url(".$val[img_path].");  height:".$gz_flasharray[$classnow][y]."px;'></a>";
						$metinfo.="
			</li>"; 
					}
					$metinfo.="
			...
			case 8:
					...
					$metinfo.="
					<div class='flash' style='height:{$gz_flasharray[$classnow][y]}px; overflow:hidden;'>
					...
					foreach($gz_flashimg as $key=>$val){
						...
						$metinfo.="
							<li>
							    <a href='".$val[img_link]."' target='_blank' title='{$val[img_title]}' class='b8' style='background-image:url({$val[img_path]}); height:{$gz_flasharray[$classnow][y]}px; '></a>";
						$metinfo.="
							</li>
						"; 
					}
		...
		switch($gz_flasharray[$classnow][imgtype]){
			case 6:
					...
					$metinfo.="<div class='flash flash6' style='width:".$gz_flasharray[$classnow][x]."px; height:".$gz_flasharray[$classnow][y]."px;'>\n";
					$metinfo.="<ul id='slider6' class='list-none'>\n";
					foreach($gz_flashimg as $key=>$val){
						$val[img_link]=str_replace('%26','&',$val[img_link]);
						$metinfo.="<li><a href='".$val[img_link]."' target='_blank' title='{$val[img_title]}'>\n";
						$metinfo.="<img src='".$val[img_path]."' alt='".$val[img_title]."' width='{$gz_flasharray[$classnow][x]}' height='{$gz_flasharray[$classnow][y]}'></a></li>\n"; 
					}
					$metinfo.="</ul>\n";
					$metinfo.="</div>\n";
...
function metlabel_foot(){
	global $gz_footright,$wap_footertext,$gz_footstat,$gz_footaddress,$gz_foottel,$gz_footother,$gz_foottext,$_M,$gz_mobileok;
	if($gz_footright<>"" or $gz_footstat<>"")$metinfo.="<p>".$gz_footright." ".$gz_footstat."</p>\n";
	if($gz_footaddress<>"")$metinfo.="<p>".$gz_footaddress."</p>\n";
	if($gz_foottel<>"")$metinfo.="<p>".$gz_foottel."</p>\n";
	if($gz_footother<>"")$metinfo.="<p>".$gz_footother."</p>\n";
	if($gz_foottext<>"")$metinfo.="<p>".$gz_foottext."</p>\n";
	if($wap_footertext<>""&&$gz_mobileok == 1)$metinfo.="<p>".$wap_footertext."</p>\n";
	//接口代码
...
function metlabel_nav($type=1,$label='',$z,$l,$home=1){
	global $index_url,$lang_home,$nav_list,$nav_list2,$nav_list3,$navdown,$lang,$db,$gz_column,$lang_cvtitle,$index,$class_list,$cv,$metinfover;
	
	if($recruitment=$db->get_one("SELECT * FROM $gz_column WHERE bigclass='0' and module='6' and lang='$lang'")){
...
function metlable_lang($dt,$tp=1,$lok=1){
	global $methtml_sethome,$methtml_addfavorite,$index_hadd_ok,$app_file,$gz_adminfile,$metinfover;
	if($metinfover == 'v1'){
...
function metlabel_navnow($type=1,$label='',$indexnum,$listyy=0,$listmax=8,$msow=0){
	global $sidedwon2,$gz_config,$sidedwon3,$index_url,$nav_list,$nav_list2,$nav_list3,$class1,$class_list,$module_list1,$class_index,$classlistall,$lang,$db,$gz_column;
	...
	switch($type){
		...
		case 4:
			...
			foreach($nav_list2[$class] as $key=>$val){
				if($val['id']!=''){
					//dump($_GET);exit;
					//$cdown=$val['id']==$sidedwon2?"class='current'":'class="cur"';
					//if($sidedwon2==10001) $cdown="class='current'";	
					//$cdown="class='cur".$val['id'].' current".$val['id'].'\'';
					$cdown='class="cur'. ' current'.$val['id'].'"';
					$i++;
					if($i!=1)$metinfo.=$label;
					$metinfo.="<li>";
					$metinfo.="<a {$cdown} href='{$val[url]}' {$val[new_windows]}   title='{$val[name]}' >{$val[name]}</a>";
					$metinfo.="</li>";
				}	
			}
			$metinfo.="</ul>";
		...
		case 5:
			foreach($nav_list2[$class] as $key=>$val){
				$nav_list2[$class][$key]['cdown']=$val['id']==$sidedwon2?"class='current'":'class="cur"';
				$nav_list2[$class][$key]['mors']=$nav_list3[$val['id']];
			}
			return $nav_list2[$class];
			break;
		case 6:

				return $class_list[$class1];
...
function membernavlist($type=0){
	global $lang,$lang_memberIndex3,$lang_memberIndex4,$lang_memberIndex5,$lang_memberIndex6,$lang_memberIndex7,$lang_memberIndex10,$app_file,$gz_adminfile,$gz_mermber_metinfo_news_left_class,$db,$gz_admin_table,$gz_weburl,$gz_adminfile,$metinfo_member_name,$gz_ifmember_left,$class_list,$navigation;
	$class=$gz_mermber_metinfo_news_left_class?$gz_mermber_metinfo_news_left_class:'membernavlist';/*兼容以前模板*/
	$admin_list = $db->get_one("SELECT * FROM $gz_admin_table WHERE admin_id='$metinfo_member_name' ");
	...
	if($type==1){
		...
		foreach($file_site as $keyfile=>$valflie){
			if(file_exists(ROOTPATH."$gz_adminfile".$valflie)&&!is_dir(ROOTPATH."$gz_adminfile".$valflie)&&((file_get_contents(ROOTPATH."$gz_adminfile".$valflie))!='metinfo')){require ROOTPATH."$gz_adminfile".$valflie;}
		}
...
function metlabel_news($time=1,$desc=0,$dlen,$dt=1,$n=0){
	global $news_list,$metblank,$id,$metinfover,$thumb_src,$gz_newsimg_x,$gz_newsimg_y;
		...
		if($style==2){
			foreach($news_list as $key=>$val){
				if($id!=$val[id]){
					...
					if($i==1)$top='top';
					$marLeft=$gz_newsimg_x+10;
					$metinfo.="
					<li class='list {$top}'>
						<a href='{$val[url]}' class='aimg' title='{$val[title]}' {$metblank}>
							<img src='{$thumb_src}dir={$val[imgurl]}&x={$gz_newsimg_x}&y={$gz_newsimg_y}' alt='{$val[title]}' class='img' />
						</a>
...
function metlabel_product($z,$w,$l,$n=0){
	global $product_list,$metblank,$gz_img_style,$gz_img_x,$gz_img_y,$gz_product_page,$class1,$class2,$class3,$search,$nav_list2,$nav_list3,$weburly,$id,$gz_agents_img,$class_list,$module_listall,$db,$lang,$gz_parameter,$gz_plist,$prices,$action,$product_para,$metinfover,$thumb_src;
	...
	$gz_img_x=$gz_img_style?gz_imgxy(1,'product'):$gz_img_x;
	$gz_img_y=$gz_img_style?gz_imgxy(2,'product'):$gz_img_y;
	
	if($metinfover=="v1"){
	
	$listarray=$product_list;
	$metok=0;
	if($gz_product_page && $search<>'search'){
	...
	foreach($listarray as $key=>$val){
		...
		if(!$n || $id!=$val[id]){
			if($metok){
				$val['title']=$val['name'];
				$val['imgurls']=$val['columnimg']==''?$weburly.$gz_agents_img:$val['columnimg'];
			}
			$metinfo.="
			        <a href='{$val[url]}' title='{$val[title]}' {$metblank} class='img'>
				        <img src='{$thumb_src}dir={$val[imgurl]}&x={$gz_img_x}&y={$gz_img_y}' alt='{$val[title]}' title='{$val[title]}' />
				        <h3>{$val[title]}</h3>
			        </a>";
...
$metinfo.="
	...
	foreach($listarray as $key=>$val){
		if(!$n || $id!=$val[id]){
			...
			$metinfo.="<a href='{$val[url]}' title='{$val[title]}' {$metblank} class='img'><img src='{$val[imgurls]}' alt='{$val[title]}' title='{$val[title]}' width='{$gz_img_x}' height='{$gz_img_y}' /></a>";
...
function metlabel_img($z,$w,$l,$n=0){
	global $img_list,$metblank,$gz_img_style,$gz_img_x,$gz_img_y,$gz_img_page,$class1,$class2,$class3,$search,$nav_list2,$nav_list3,$weburly,$id,$class_list,$module_listall,$metinfover,$thumb_src;
	$gz_img_x=$gz_img_style?gz_imgxy(1,'img'):$gz_img_x;
	$gz_img_y=$gz_img_style?gz_imgxy(2,'img'):$gz_img_y;
	
	if($metinfover=="v1"){

	$listarray=$img_list;
	$metok=0;
	if($gz_img_page && $search<>'search'){
		if($class2 && count($nav_list3[$class2]) && !$class3){
			$listarray=$nav_list3[$class2];
			$metok=1;
		}
...
function metlabel_job($type){
	...
	if($type==1){
		foreach($job_list as $key=>$val){
			...
			$metinfo.="<div class='editor gz_editor'>{$val[content]}</span></div>";
...
function messagelabel_table($dy){
	...
	global $gz_memberlogin_code,$lang_memberImgCode,$lang_memberTip1,$gz_adminfile,$navurl,$settings_arr;
	global $db,$gz_parameter,$gz_admin_array,$gz_member_use,$metinfo_member_type,$gz_list,$gz_class,$class_list,$gz_product,$lang_Choice,$lang_Empty,$paravalue;
	if($fid)$id=$fid;
	if(!$title){
	...
	$query = "SELECT * FROM $gz_parameter where lang='$lang' and  module=7 order by no_order";	
	...
	while($list= $db->fetch_array($result)){
		if($list[type]==2 or $list[type]==4 or $list[type]==6){
			...
			if($listinfo){
				...
				$query1 = "select * from $gz_product where lang='$lang' and $classtype='$listinfoid' and recycle='0' order by updatetime desc";
...
function metlabel_messagelist(){
	global $lang,$message_list,$lang_SubmitContent,$lang_Reply,$total_count,$from_record,$db,$gz_config,$gz_mlist,$gz_message_fd_class,$gz_message_fd_content;
	$c=$total_count-$from_record;
	foreach($message_list as $key=>$val){
		$key=$key+1;
	$messagesNames1=$db->get_one("select * from $gz_mlist where listid='$val[id]' and paraid='$gz_message_fd_class' and lang='$lang'");
	// print_r($message_list);die;
	$messagesNames2=$db->get_one("select * from $gz_mlist where listid='$val[id]' and paraid='$gz_message_fd_content' and lang='$lang'");
	// $metinfo.="<dl class='list-none metlist'>\n";
	...
	$metinfo.="<div class='zxzt'>发表时间：<span class='lynr'>{$val[addtime]}</span></div>\n";
	$metinfo.="<div class='zxzt'>意见内容：<span class='lynr'>{$val[content]}</span></div>\n";
	$metinfo.="<div class='zxzt'>意见回复：<span class='lynr'>{$val[useinfo]}</span></div>\n";
	$metinfo.="<div class='xbkxx'></div> \n";
...
function metlabel_member($fid,$mobile){ 
	global $lang,$message_list,$lang_Submit,$lang_Reset,$lang_Publish,$lang_Reply,$fromurl,$m_user_ip,$id,$title;
	global $gz_memberlogin_code,$lang_memberImgCode,$lang_memberTip1,$gz_adminfile,$navurl,$settings_arr;
	global $db,$lang_memberRegisterName,$lang_js7,$lang_js26,$lang_memberPassword,$lang_js8,$lang_js9,$lang_js10,$lang_js12,$lang_js11,$lang_js13,$lang_memberName,$lang_membereditorPs,$lang_membereditorPs1,$lang_Email,$lang_memberbasicCompanyName,$gz_parameter,$gz_admin_array,$gz_member_use,$metinfo_member_type,$gz_list,$gz_class,$class_list,$gz_product,$lang_Choice,$lang_Empty,$lang_memberRegister,$paravalue;
	if($fid)$id=$fid;
	if(!$title){
		foreach($settings_arr as $key=>$val){
			if($val['columnid']==$id && $val['name']=='gz_fdtable'){
	...
	$query = "SELECT * FROM $gz_parameter where lang='$lang' and  module=10 and wr_oks='1' order by no_order";
	if($gz_member_use)$query = "select * from $gz_parameter where (access in(select id from $gz_admin_array where user_webpower<='$metinfo_member_type') or access=0) and lang='$lang' and module=10 and wr_oks='1' order by no_order;";
	$result = $db->query($query);
	while($list= $db->fetch_array($result)){
	 if($list[type]==2 or $list[type]==4 or $list[type]==6){
		$listinfo=$db->get_one("select * from $gz_list where bigid='$list[id]' and no_order=99999");
...
function metlabel_message($fid,$mobile){
	...
	$query = "SELECT * FROM $gz_parameter where lang='$lang' and  module=7 and class1='$id' order by no_order";
	if($gz_member_use)$query = "select * from $gz_parameter where (access in(select id from $gz_admin_array where user_webpower<='$metinfo_member_type') or access=0) and lang='$lang' and module=7 and class1='$id' order by no_order;";
	$result = $db->query($query);
	while($list= $db->fetch_array($result)){
	 if($list[type]==2 or $list[type]==4 or $list[type]==6){
		$listinfo=$db->get_one("select * from $gz_list where bigid='$list[id]' and no_order=99999");
...
function metlabel_messageold($fid,$mobile){
	...
	global $gz_memberlogin_code,$lang_memberImgCode,$lang_memberTip1,$gz_adminfile,$navurl,$settings_arr;
	global $db,$gz_parameter,$gz_admin_array,$gz_member_use,$metinfo_member_type,$gz_list,$gz_class,$class_list,$gz_product,$lang_Choice,$lang_Empty;		
	if($fid)$id=$fid;
	if(!$title){
		foreach($settings_arr as $key=>$val){
			if($val['columnid']==$id && $val['name']=='gz_fdtable'){
...
function metlabel_addlink($tt=1){
	global $lang_Info4,$lang_LinkInfo2,$lang_LinkInfo3,$lang_OurWebName,$gz_linkname,$lang_OurWebUrl,$gz_weburl,$lang_OurWebLOGO,$gz_logo,$lang_OurWebKeywords,$gz_keywords,$lang_YourWebName,$lang_YourWebUrl,$lang_LinkType,$lang_TextLink,$lang_PictureLink,$lang_YourWebLOGO,$lang_YourWebKeywords,$lang_Contact,$lang_Submit,$lang_Reset,$lang;
	global $gz_memberlogin_code,$lang_memberImgCode,$lang_memberTip1,$gz_adminfile;
	if($metinfover){
	...
	$metinfo.="<td class='input'>{$gz_linkname}</td></tr>\n";
	$metinfo.="<tr><td class='text'>{$lang_OurWebUrl}</td>\n";
	$metinfo.="<td class='input'>{$gz_weburl}</td></tr>\n";
	$metinfo.="<tr><td class='text'>{$lang_OurWebLOGO}</td>\n";
	$metinfo.="<td class='input'><img src='{$gz_logo}' alt='{$lang_OurWebName}' title='{$lang_OurWebName}' /></td></tr>\n";
	$metinfo.="<tr><td class='text'>{$lang_OurWebKeywords}</td>\n";
	$metinfo.="<td class='input'>{$gz_keywords}</td></tr>\n";
...
function metlabel_cv($mobile=0){
	global $fdjs,$lang,$lang_Nolimit,$lang_memberPosition,$selectjob,$cv_para,$paravalue,$gz_memberlogin_code,$lang_memberImgCode,$lang_memberTip1,$lang_Submit,$lang_Reset,$gz_adminfile,$metinfover;
	if($metinfover){
...
function sitemaplist(){
	global $db,$nav_listall,$m_now_date,$gz_sitemap_not1,$gz_sitemap_not2,$lang,$gz_langok,$gz_index_url,$gz_webname,$gz_weburl;
	global $gz_config,$langmark,$class_list,$gz_index_type,$gz_pseudo,$gz_webhtm,$gz_htmtype,$gz_htmpagename,$gz_listhtmltype,$gz_htmlistname,$gz_chtmtype,$metadmin;
	
	$gz_webname=$db->get_one("select * from $gz_config where name='gz_webname' and lang='$lang'");
	$gz_webname=$gz_webname[value];
	
	$gz_weburl=$db->get_one("select * from $gz_config where name='gz_weburl' and lang='$lang'");
	$gz_weburl=$gz_weburl[value];
	
	$gz_pseudo=$db->get_one("select * from $gz_config where name='gz_pseudo' and lang='$lang'");
	$gz_pseudo=$gz_pseudo[value];
	
	$gz_webhtm=$db->get_one("select * from $gz_config where name='gz_webhtm' and lang='$lang'");
	$gz_webhtm=$gz_webhtm[value];
	
	$gz_htmtype=$db->get_one("select * from $gz_config where name='gz_htmtype' and lang='$lang'");
	$gz_htmtype=$gz_htmtype[value];
	
	$gz_htmpagename=$db->get_one("select * from $gz_config where name='gz_htmpagename' and lang='$lang'");
	$gz_htmpagename=$gz_htmpagename[value];
	
	$gz_listhtmltype=$db->get_one("select * from $gz_config where name='gz_listhtmltype' and lang='$lang'");
	$gz_listhtmltype=$gz_listhtmltype[value];
	
	$gz_htmlistname=$db->get_one("select * from $gz_config where name='gz_htmlistname' and lang='$lang'");
	$gz_htmlistname=$gz_htmlistname[value];
	if($gz_index_type==$lang){
		$gz_chtmtype='.'.$gz_htmtype;
		$gz_htmtype='.'.$gz_htmtype;
	}else{
		$gz_chtmtype='_'.$lang.'.'.$gz_htmtype;
		$gz_htmtype='_'.$lang.'.'.$gz_htmtype;
	}
	$langmark="lang=$lang";
	//include ROOTPATH.'include\lang.php';
	$indexar=array('title'=>$gz_webname,'url'=>$gz_index_url[$lang],'updatetime'=>date("Y-m-d"),'priority'=>1);
	$nav_listall=array();
	include ROOTPATH.'include/global/pseudo.php';
	$sitemaplist[]=$indexar;
	foreach($nav_listall as $key=>$val){
		$no1ok=$val[nav]?1:($gz_sitemap_not1 && !$val['bigclass']?0:1);
		$no2ok=$val[if_in]==0?1:($gz_sitemap_not2?0:1);
		if($val[module]!=10 && $val[module]!=11 && $no1ok && $no2ok && $val[isshow]==1){
			...
			$val[url]=$val[if_in] ? $val[url] : $gz_weburl.$val[url];
			$sitemaplist[]=$val;
		}
	}

	foreach(methtml_getarray('','all','time','news',50000) as $key=>$val){
		if($gz_sitemap_not2 == 1 && $val[links])continue;
		$val[url]=str_replace('..//','',$val[url]);
		$val[url]=str_replace('../','',$val[url]);
		$val[url]= $val[links] ? $val[links] : $gz_weburl.$val[url];
...
include_once ROOTPATH.$gz_adminfile.'/app/wap/menu_map.php';
//接口
if(strstr($_SERVER['PHP_SELF'], 'gz_shop'))$special = 1;
...
function paging($p,$num,$numbers,$sum,$url){//p 当前页码，num 显示条数,numbers，sum 总记录数， 固定页码数,url 跳转路径,
if(!empty($p)){
	$p=$p*1;
}else
{
...
//$num=2;//显示条数
//$numbers=3;//设定好页码
$nums=floor($numbers/2); //偏幅量
$startpage = $p - $nums;//当前页-偏幅量
$endpage = $p+ $nums;//当前页+偏幅量
$offset=($p-1)*$num; //偏移量
$limit="$offset,$num";
//$sql=mysql_query("SELECT count(*) FROM $table where $where order by $sort desc limit $shu ");
...
$ps=$p-1;
if($ps<1){$ps=1;}	
$number.="<a href='$url&page=".$ps."'>上一页</a>";
...
$number.="<a href='$url&page=".$i."'>$i</a>";
}

}

$ps1=$p+1;
if($ps1>$pages){$ps1=$pages;}	
$number.="<a href='$url&page=".$ps1."'>下一页</a>";
$variable=array(
	'number'=>$number,
	'limit'=>$limit
	);
