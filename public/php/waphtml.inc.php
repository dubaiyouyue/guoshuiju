...
function wap_foot_menu(){
	global $weburly,$gz_menu_ok,$gz_menu_oks,$gz_menu_rgb,$gz_menu_bg,$gz_title,$show,$m_now_year,$navurl,$gz_js_access,$gz_skin_css,$img_url,$gz_webname,$metcms_v,$appscriptcss,$gz_ch_lang,$lang,$gz_ch_mark,$gz_url,$metinfouiok,$classnow,$class_list,$gz_headstat;
	global $gz_wap,$gz_wap_tpa,$gz_wap_tpb,$gz_webhtm,$gz_wap_url,$module,$metinfonow,$gz_member_force,$gz_weburl,$gz_wapshowtype,$lang_foottext1,$lang_foottext2,$lang_foottext3,$lang_foottext4,$lang_footnav_tel,$index_url,$index;
	global $db,$gz_wapmenu,$metinfover;
	$menu_ok=$gz_menu_ok;
	$types=$gz_menu_oks;
	$textbg=$gz_menu_rgb;
	if($gz_menu_bg==$textbg&&$gz_menu_bg){
		$gz_menu_bg=str_replace("../","",$gz_menu_bg);
		$gz_menu_bgs="style='background-image:url(".$weburly.$gz_menu_bg.")'";
		$gz_sioncss="#footer ul li{ display:-webkit-box; -webkit-box-flex:1; text-align:center; border-right:0px solid rgba(255,255,255,0.2); border-left:0px;}";
	}else{
		$gz_menu_bgs="";
		$gz_sioncss="";
		}
	...
	if($menu_ok==1){
		if($types==1){
			...
			foreach($menurs as $key=>$val){
				...
				if($val[type]==3){
					if($metinfover){
						$metinfo.="<div style='background:{$textbg} !important;'> <a href='{$navurl}index.php?lang=$lang&map=1&gz_mobileok=1&uid={$val[id]}'><i class='fa fa-map-marker' style='color:{$val[menu_iconrgb]}; font-size:25px; line-height:32px;'></i></a> </div>";
					}else{
						$metinfo.="<div style='background:{$textbg} !important;'> <a href='{$navurl}index.php?lang=$lang&map=1&gz_mobileok=1&uid={$val[id]}'><span class='icon-map-marker' style='color:{$val[menu_iconrgb]};'></span></a> </div>";
					}
				}
				if($val[type]==4){
					if($metinfover){
		...
		if($types==2){
			...
			<div class='footerbox'>
			<div class='footerlist' {$gz_menu_bgs}>
			<ul>";
			foreach($menurs as $key=>$val){
				if($val[type]==1){
					if($metinfover){
				...
				if($val[type]==3){
					if($metinfover){
						$metinfo.="<li style='width:100%;'><a href='{$navurl}index.php?lang=$lang&map=1&gz_mobileok=1&uid={$val[id]}'><i class='icon fa fa-map-marker' style='color:{$val[menu_iconrgb]}; '></i><span class='txt' style='color:{$val[menu_wordrgb]};'>{$val[name]}</span></a></li>";
					}else{
					$metinfo.="<li style='width:100%;'><a href='{$navurl}index.php?lang=$lang&map=1&gz_mobileok=1&uid={$val[id]}'><span class='icon  icon-map-marker' style='color:{$val[menu_iconrgb]}; font-size:25px;'></span><span class='txt' style='color:{$val[menu_wordrgb]};'>{$val[name]}</span></a></li>";
					}
				}
				if($val[type]==4){
					if($metinfover){
