...
function metlabel_conjunctive($field,$price,$region,$img){
	global $db,$lang,$gz_parameter,$gz_plist,$gz_weburl,$quirys,$prices,$product_para,$para_select,$class1,$lang_cvall;
	if(!$class1){
		$class1=10001;
	}
	...
	if($field){
		...
		foreach($para_product_list as $key=>$val){
			...
			foreach($product_para_id as $key=>$val1){
				...
				$conjunctive[$val1[name]][all][url]=$gz_weburl."product/product.php?lang=".$lang.$quiryx;
				...
				$conjunctive[$val1[name]][$val4][url]=$gz_weburl."product/product.php?lang=".$lang.$quiryx;
				}
...
global $lang_AllBigclass,$navurl,$lang,$lang_AllThirdclass,$lang_AllSmallclass,$lang_search,$lang_Nolimit,$img_url,$metinfo_member_type,$gz_member_use;
...
if($metinfo_member_type>=intval($val[access])&&$gz_member_use){
switch($val[type]){
 case 1:
  $k1++;
...
function title($class1,$anyid,$lang){
	global $gz_class1,$gz_class2,$gz_weburl,$gz_adminfile,$gz_content_type;
	if($gz_content_type==1){
	foreach ($gz_class1 as $val) {
		$idall[]=$val[id];
	}
	if(!in_array($class1, $idall)&&$class1!=0){
		foreach ($gz_class2 as $val1) {
		foreach ($val1 as $val2) {
			if($val2[id]==$class1){
				$classid1=$val2[bigclass];
				$classname1=$gz_class1[$classid1][name];
				$module1=$gz_class1[$classid1][module];
				$classname2=$val2[name];
			}
		}
		}
	$title="<a href='{$gz_weburl}{$gz_adminfile}/content/content.php?anyid={$anyid}&lang={$lang}&module=$module1&class1={$classid1}'>{$classname1}</a>><a href='JavaScript:;'>$classname2</a>";
	}	
	}
	return $title;
}
