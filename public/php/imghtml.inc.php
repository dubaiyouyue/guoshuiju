...
function methtml_img($listtype,$type,$titlenum,$paranum,$detail,$feedback,$time,$hits,$newwindow=1,$desription,$desnum,$classname,$news,$hot,$top,$listnav=1,$max,$topcolor){
 ...
 global $img_para,$img_paralist,$gz_img_x,$gz_img_y,$addfeedback_url,$gz_img_page;
 ...
 $metimgok=0;
 if($gz_img_page && $listtype=='img' &&($search<>'search')){
...
if($listtype=='img'){
 ...
 if($metimgok){
 $listtext.=" ".$val[new_windows]."><img src=".$val[columnimg]." alt=".$val[name]." width=".$gz_img_x." height=".$gz_img_y." /></a></span>";
...
function methtml_showimg($type,$feedback=0,$desription){
 ...
 global $imgpara,$img_paralist,$gz_img_x,$gz_img_y,$addfeedback_url,$gz_url,$img_paraimg;
