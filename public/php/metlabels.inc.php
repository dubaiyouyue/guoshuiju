...
function metlabel_head($closure=1,$iehack=1,$mobileto=''){
	global $gz_mobileok,$gz_title,$show,$classnow,$id,$class_list,$navurl,$gz_js_access,$img_url;
	global $appscriptcss;
	global $_M;
	$gz_skin_css = $_M['config']['et_skin_css']==''?'metinfo.css':$_M['config']['et_skin_css'];
	$closure = $closure?"\n</head>":'';
	if($gz_mobileok){
		$metinfo="
<!DOCTYPE HTML>
...
<meta name=\"generator\" content=\"MetInfo {$_M[config][metcms_v]}\"  data-variable=\"{$_M[config][gz_weburl]}|{$_M[lang]}|{$classnow}|{$id}|{$class_list[$classnow][module]}|{$_M[config][gz_skin_user]}|mobile\" />
<meta content=\"yes\" name=\"apple-mobile-web-app-capable\" />
<meta content=\"black\" name=\"apple-mobile-web-app-status-bar-style\" />
<meta content=\"telephone=no\" name=\"format-detection\" />
<link href=\"favicon.ico\" rel=\"apple-touch-icon-precomposed\" />
<link href=\"favicon.ico\" rel=\"shortcut icon\" type=\"image/x-icon\" />
<link rel=\"stylesheet\" type=\"text/css\" href=\"{$img_url}css/{$gz_skin_css}\" />{$gz_js_access}{$closure}";
	}else{
...
<meta http-equiv=\"X-UA-Compatible\" content=\"IE=edge,chrome=1\">
<meta content=\"width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0\" name=\"viewport\" />
<meta name=\"generator\" content=\"MetInfo {$_M[config][metcms_v]}\"  data-variable=\"{$_M[config][gz_weburl]}|{$_M[lang]}|{$classnow}|{$id}|{$class_list[$classnow][module]}|{$_M[config][gz_skin_user]}\" />
<link href=\"{$navurl}favicon.ico\" rel=\"shortcut icon\" />
<link rel=\"stylesheet\" type=\"text/css\" href=\"{$img_url}css/{$gz_skin_css}\" />
{$_M['html_plugin']['head_script']}{$appscriptcss}{$iehack}{$gz_js_access}{$_M[config][gz_headstat]}{$closure}";
	}
	return $metinfo;
}
function metlabel_topnav($dt='',$tp=1,$lok=1){
	global $gz_langok,$gz_langok,$gz_lang_mark,$gz_index_url,$app_file,$gz_adminfile,$gz_index_type,$index_url,$navurl,$gz_lang_mark,$gz_wap_url;
	global $_M;
	/*手机版链接*/
	if($_M['config']['gz_waplink'] && $_M['config']['gz_wap']){
		if($gz_wap_url){
			$indurl = $gz_wap_url;
		}else{
			$indurl = $gz_index_type==$_M[lang]?$index_url.'wap/':$navurl.'wap/index.php?lang='.$_M[lang];
			if($_M['config']['gz_wap_tpb']&&$_M['config']['gz_wap_url'])$indurl = $_M['config']['gz_wap_url'];
		}
		$mobile = "<li><a href='{$indurl}' title='{$_M['word']['wap']}'>{$_M['word']['wap']}</a></li>{$dt}";
	}
	/*JS繁体中文*/
	if($_M['config']['gz_ch_lang'] and $_M['lang']=='cn'){
		if(count($gz_langok)>1&&$gz_lang_mark){
	...
	if($lok&&$gz_lang_mark){
		foreach($gz_langok as $key=>$val){
			...
			if($val['useok'])$langlist.="{$dt}<li><a href='{$gz_index_url[$val[mark]]}' {$urlnew}>{$flag}{$val[name]}</a></li>";
		}
	}
...
function metlabel_form($list,$type){
	global $fdjs,$lang,$lang_Nolimit,$lang_memberPosition,$selectjob,$cv_para,$paravalue,$gz_memberlogin_code,$lang_memberImgCode,$lang_memberTip1,$lang_Submit,$navurl;
	$lista=array(); 
