...
$methtml_head.="<title>".$gz_title."</title>\n";
$methtml_head.="<meta name=\"description\" content=\"".$show['description']."\" />\n";
$methtml_head.="<meta name=\"keywords\" content=\"".$show['keywords']."\" />\n";
$methtml_head.="<meta name=\"author\" content=\"{$gz_webname}\" />\n";
$methtml_head.="<meta name=\"copyright\" content=\"Copyright 2012-".$m_now_year." MetInfo\" />\n";
$methtml_head.="<link href=\"".$navurl."favicon.ico\" rel=\"shortcut icon\" />\n";
if($gz_js_access)$methtml_head.=$gz_js_access."\n";
//memberjs
if($gz_skin_css=='')$gz_skin_css='metinfo.css';
$methtml_head.="<link rel=\"stylesheet\" type=\"text/css\" href=\"".$img_url."css/".$gz_skin_css."\" />\n";
$methtml_head.="<script src=\"".$gz_url."js/metinfo-min.js\" type=\"text/javascript\"></script>\n";
if($gz_ch_lang and $lang==$gz_ch_mark)$methtml_head.="<script src=\"".$gz_url."js/ch.js\" type=\"text/javascript\"></script>\n";
//style
...
$query = "SELECT * FROM gz_config where lang='$lang' and name='gz_headstat'";
...
global $lang,$lang_chchinese,$gz_ch_mark,$gz_ch_lang,$gz_langok,$gz_url,$index_url,$gz_index_url,$gz_lang_mark,$gz_waplink,$lang_wap,$gz_wap_tpb,$gz_wap_url,$gz_wap,$gz_index_type,$navurl,$app_file,$gz_adminfile,$_M;
$metinfo='';
if($gz_lang_mark){
	switch($type){
	case 1:
	...
	if($val[useok] and $val[mark]!=$lang)$metinfo.="<a href='".$gz_index_url[$val[mark]]."' title='$val[name]' $urlnew >".$val[name]."</a>".$label;
	}
	if($gz_waplink && $gz_wap){
		if($gz_wap_tpb&&$gz_wap_url){
			$metinfo="<a href='$gz_wap_url' title='{$lang_wap}'>{$lang_wap}</a>".$label.$metinfo;
		}
		else{
			$indurl=$gz_index_type==$lang?$index_url.'wap/':$navurl.'wap/index.php?lang='.$lang;
			$metinfo="<a href='{$indurl}' title='{$lang_wap}'>{$lang_wap}</a>".$label.$metinfo;
		}
	}
	if($gz_ch_lang and $lang==$gz_ch_mark){
	...
	foreach($gz_langok as $key=>$val){
	$urlnew=$val[newwindows]?"target='_blank'":"";
	if($val[useok] and $val[mark]!=$lang)$metinfo.="<a href='".$gz_index_url[$val[mark]]."' title='$val[name]' $urlnew ><img src='$val[flag]' border='0' /></a>".$label;
	}
	if($gz_ch_lang and $lang==$gz_ch_mark){
	$metinfo="<a class=\"fontswitch\" id=\"StranLink\" href=\"javascript:StranBody()\"><img src='".$gz_langok[$gz_ch_mark][flag]."' border='0' /></a>".$label.$metinfo;
	}
	break;
	}
	...
	if($gz_waplink && $gz_wap){
		if($gz_wap_tpb&&$gz_wap_url){
			$metinfo="<a href='$gz_wap_url' title='{$lang_wap}'>{$lang_wap}</a>";
		}else{
			$indurl=$gz_index_type==$lang?$index_url.'wap/':$navurl.'wap/index.php?lang='.$lang;
	...
	foreach($file_site as $keyfile=>$valflie){
		if(file_exists(ROOTPATH."$gz_adminfile".$valflie)&&!is_dir(ROOTPATH."$gz_adminfile".$valflie)){require ROOTPATH."$gz_adminfile".$valflie;}
	}
	//接口代码
...
//2016/12/07
 // <a href="#" style="color:#333;">公众互动</a> > <a href="#" style="color:#E77817;">意见征集</a></P>
$navx_css="style='color:#333;'";
if($class2<>0){$navx_css1="style='color:#E77817;'";}
if($class3<>0){$navx_css2="style='color:#E77817;'";$navx_css1=$navx_css;}
$nav_x[name]="<a $navx_css href=".$class1_info[url]." >".$class1_info[name]."</a>";
if($class2<>0){

$nav_x[name]=$nav_x[name]." > "."<a  $navx_css1 href=".$class2_info[url]." >".$class2_info[name]."</a>";
}
if($class3<>0){
$nav_x[name]=$nav_x[name]." > "."<a $navx_css2 href=".$class3_info[url]." >".$class3_info[name]."</a>";
}
$nav_x[1]=$nav_x[name];
$nav_x[2]="<a href=".$class_list[$classnow][url]." >".$class_list[$classnow][name]."</a>";
...
global $navlist_y1,$class1,$class2,$class3,$classnow,$nav_list2,$nav_list3,$class_index,$module_list1,$class_list;
  ...
  foreach($nav2 as $key=>$val){
	...
	if($class3ok)$metinfo.="onmousedown='gz_showhide1".$mark."(".$val[id].")'";
  ...
  if($class3ok){
    $metinfo.="function gz_showhide1".$mark."(d)\n";
    $metinfo.="{  \n";     
...
global $gz_flasharray,$classnow,$gz_url,$gz_flash_img,$gz_flash_imglink,$gz_flash_imgtitle,$gz_flashimg,$metinfover;
if($width=='')$width=$gz_flasharray[$classnow][x];
if($height=='')$height=$gz_flasharray[$classnow][y];
if($imgurl=='')$imgurl=$gz_flash_img;
if($imglink=='')$imglink=$gz_flash_imglink;
if($imgtitle=='')$imgtitle=$gz_flash_imgtitle;
$imglink=str_replace('&','%26',$imglink);
if($metinfover=='v1'){
switch($type){
case 1:
	$methtml_flash.="
	<div class='slider-wrapper metinfo-banner1' style='height:{$gz_flasharray[$classnow][y]}px; width:{$gz_flasharray[$classnow][x]}px;'>
	...
	foreach($gz_flashimg as $key=>$val){
		...
		$methtml_flash.="<img src='".$val[img_path]."' alt='".$val[img_title]."' width='{$gz_flasharray[$classnow][x]}' height='{$gz_flasharray[$classnow][y]}' />"; 
...
case 3:
	$methtml_flash.="	<div class='slider-wrapper metinfo-banner3' style='height:{$gz_flasharray[$classnow][y]}px; width:{$gz_flasharray[$classnow][x]}px;'>
	<div id=\"slider\" class=\"nivoSlider\">\n";
	foreach($gz_flashimg as $key=>$val){
		$val[img_link]=str_replace('%26','&',$val[img_link]);
...
case 4:
	$methtml_flash.="	<div id='metinfo_banner4' style='height:{$gz_flasharray[$classnow][y]}px;'>
	";
	$methtml_flash.="    <ul>
	";
	foreach($gz_flashimg as $key=>$val){
		$methtml_flash.="    	<li>";
		$val[img_link]=str_replace('%26','&',$val[img_link]);
		if($val[img_link])$methtml_flash.="<a href='".$val[img_link]."' style='background-image:url({$val[img_path]}); height:".$gz_flasharray[$classnow][y]."px;' target='_blank' title='{$val[img_title]}'>";
		if($val[img_link])$methtml_flash.="</a>";
		$methtml_flash.="</li>
	";
	}
...
case 5:
	$methtml_flash.="
		<div class='slider-wrapper metinfo-banner5' style='height:{$gz_flasharray[$classnow][y]}px;'>
			<div id='slider' class='nivoSlider'>
	";
	foreach($gz_flashimg as $key=>$val){
		$val[img_link]=str_replace('%26','&',$val[img_link]);
...
case 6:
	$img_x=$gz_flasharray[$classnow][x];
	$methtml_flash.="
		<div id='viewport-shadow' class='trans metinfo-banner7' style='height:{$gz_flasharray[$classnow][y]}px;'>";
	$methtml_flash.="
	...
	foreach($gz_flashimg as $key=>$val){
		...
		if($val[img_link])$methtml_flash.="
			    <a href='".$val[img_link]."' target='_blank' title='{$val[img_title]}'>";
		$methtml_flash.="
			        <img src='".$val[img_path]."' alt='".$val[img_title]."' width='{$img_x}' height='{$gz_flasharray[$classnow][y]}' />"; 
		if($val[img_link])$methtml_flash.="
			    </a>";
		$methtml_flash.="
			</figure>";
...
switch($type){
case 1:
	$methtml_flash.="<link rel=\"stylesheet\" href=\"{$gz_url}banner/nivo-slider/nivo-slider.css\" type=\"text/css\" media=\"screen\" />
<script type=\"text/javascript\" src=\"{$gz_url}banner/nivo-slider/jquery.nivo.slider.pack.js\"></script>
	<style type=\"text/css\">.metinfo-banner1 img{ height:{$gz_flasharray[$classnow][y]}px!important;}</style>
	";
	$methtml_flash.="<div class=\"slider-wrapper metinfo-banner1\" style=\"height:{$gz_flasharray[$classnow][y]}px;\">
	<div id=\"slider\" class=\"nivoSlider\">\n";
	foreach($gz_flashimg as $key=>$val){
		$val[img_link]=str_replace('%26','&',$val[img_link]);
		if($val[img_link])$methtml_flash.="<a href='".$val[img_link]."' target='_blank' title='{$val[img_title]}'>";
...
case 2:
   ...
   $methtml_flash.="var swfpath = '".$gz_url."'+'banner/flash02.swf';\n";
...
case 3:
	$methtml_flash.="
	<link rel=\"stylesheet\" href=\"{$gz_url}banner/nivo-slider/nivo-slider.css\" type=\"text/css\" media=\"screen\" />
	<script type=\"text/javascript\" src=\"{$gz_url}banner/nivo-slider/jquery.nivo.slider.pack.js\"></script>
	<style type=\"text/css\">.metinfo-banner3 img{ height:{$gz_flasharray[$classnow][y]}px!important;}</style>
	";
	$methtml_flash.="<div class=\"slider-wrapper metinfo-banner3\" style=\"height:{$gz_flasharray[$classnow][y]}px;\">
	<div id=\"slider\" class=\"nivoSlider\">\n";
	foreach($gz_flashimg as $key=>$val){
		$val[img_link]=str_replace('%26','&',$val[img_link]);
		if($val[img_link])$methtml_flash.="<a href='".$val[img_link]."' target='_blank' title='{$val[img_title]}'>";
...
case 4:
	$methtml_flash.="
	<link rel=\"stylesheet\" href=\"{$gz_url}banner/banner4/style.css\" type=\"text/css\" media=\"screen\" />
	<script type=\"text/javascript\" src=\"{$gz_url}banner/banner4/flash4.js\"></script>
	";
	$methtml_flash.="<div id=\"metinfo_banner4\" style=\"height:{$gz_flasharray[$classnow][y]}px;\">\n";
	$methtml_flash.="<ul>\n";
	foreach($gz_flashimg as $key=>$val){
...
case 5:
	...
	<link rel=\"stylesheet\" href=\"{$gz_url}banner/nivo-slider/nivo-slider.css\" type=\"text/css\" media=\"screen\" />
	<script type=\"text/javascript\" src=\"{$gz_url}banner/nivo-slider/jquery.nivo.slider.pack.js\"></script>
	<style type=\"text/css\">.metinfo-banner5 img{ height:{$gz_flasharray[$classnow][y]}px!important;}</style>
	";
	$methtml_flash.="<div class=\"slider-wrapper metinfo-banner5\" style=\"height:{$gz_flasharray[$classnow][y]}px;\">
	<div id=\"slider\" class=\"nivoSlider\">\n";
	foreach($gz_flashimg as $key=>$val){
		$val[img_link]=str_replace('%26','&',$val[img_link]);
		if($val[img_link])$methtml_flash.="<a href='".$val[img_link]."' target='_blank' title='{$val[img_title]}'>";
...
case 6:
	$img_x=$gz_flasharray[$classnow][x];
	$methtml_flash.="
	<link rel=\"stylesheet\" href=\"{$gz_url}banner/banner7/style.css\" type=\"text/css\" media=\"screen\" />
	<script type=\"text/javascript\" src=\"{$gz_url}banner/banner7/modernizr.min.js\"></script>
	<script type=\"text/javascript\" src=\"{$gz_url}banner/banner7/box-slider-all.jquery.min.js\"></script>
	";
	$methtml_flash.="<div id=\"viewport-shadow\" class=\"trans metinfo-banner7\" style=\"height:{$gz_flasharray[$classnow][y]}px;\">\n";
	...
	foreach($gz_flashimg as $key=>$val){
		...
		$val[img_link]=str_replace('%26','&',$val[img_link]);
		if($val[img_link])$methtml_flash.="<a href='".$val[img_link]."' target='_blank' title='{$val[img_title]}'>";
		$methtml_flash.="<img src='".$val[img_path]."' alt='".$val[img_title]."' width='{$img_x}' height='{$gz_flasharray[$classnow][y]}' />"; 
		if($val[img_link])$methtml_flash.="</a>";
...
//Flash
$methtml_flash="";
switch($gz_flasharray[$classnow][type]){
	case 0:
		$methtml_flash="";
	break;
	case 1:
		switch($gz_flasharray[$classnow][imgtype]){
			case 1:
			...
			case 6:
				...
				$methtml_flash.="<div class='flash flash6' style='width:".$gz_flasharray[$classnow][x]."px; height:".$gz_flasharray[$classnow][y]."px;'>\n";
				$methtml_flash.="<ul id='slider6' class='list-none'>\n";
				foreach($gz_flashimg as $key=>$val){
					$methtml_flash.="<li><a href='".$val[img_link]."' target='_blank' title='{$val[img_title]}'>\n";
					$methtml_flash.="<img src='".$val[img_path]."' alt='".$val[img_title]."' width='{$gz_flasharray[$classnow][x]}' height='{$gz_flasharray[$classnow][y]}'></a></li>\n";
				}
				$methtml_flash.="</ul>\n";
				$methtml_flash.="</div>\n";
				$methtml_flash.="<script type='text/javascript'>
			...
			break;
			case 8:
				$thisflash_x=$gz_flasharray[$classnow][x]-8;
				$thisflash_y=$gz_flasharray[$classnow][y]-8;
...
<script src='{$navurl}public/jq-flexslider/jquery.flexslider-min.js'></script>";
	...
	case 2:
		if($gz_flash_url<>''){
			$methtml_flash.="<div class=\"flash\" style=\"background-image: url(".$gz_flash_back."); width:".$gz_flash_x."px; height:".$gz_flash_y."px;\">\n";
			$methtml_flash.="<object classid=\"clsid:D27CDB6E-AE6D-11cf-96B8-444553540000\" codebase=\"http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,29,0\" width=\"".$gz_flash_x."\" height=\"".$gz_flash_y."\">\n";
			$methtml_flash.="            <param name=\"movie\" value=\"".$gz_flash_url."\">\n";
			$methtml_flash.="            <param name=\"quality\" value=\"high\">\n";
			$methtml_flash.="            <param name=\"wmode\" value=\"transparent\" />\n";
			$methtml_flash.="		 <embed src=\"".$gz_flash_url."\" width=\"".$gz_flash_x."\" height=\"".$gz_flash_y."\" quality=\"high\" wmode=\"transparent\"\n";
	...
	case 3:
		if($flash_imgone_img<>''){
			...
			$methtml_flash.="<img src='".$flash_imgone_img."' width='".$gz_flasharray[$classnow][x]."' alt='$flash_imgone_title' height='".$gz_flasharray[$classnow][y]."'>";
...
function methtml_getarray($mark,$type,$order,$module,$listmx=-1,$para=0,$categoryname=0,$marktype=0,$txtmax=0,$descmax=0,$rand=0){
	global $gz_member_use,$class_index,$gz_listtime,$metinfo_member_type,$db,$gz_news,$gz_product,$gz_download,$gz_img,$gz_job,$gz_parameter,$gz_plist,$class_list,$metpara,$module_list2;
	global $index_news_no,$index_product_no,$index_download_no,$index_img_no,$index_job_no,$mobilesql;
	global $index,$navurl,$weburly,$lang,$pagename,$langmark,$gz_htmpagename,$gz_chtmtype,$gz_htmtype,$gz_pseudo,$gz_webhtm;
	global $dataoptimize,$pagemark,$img_url,$gz_hot,$m_now_date,$gz_newsdays,$metmemberforce,$gz_alt,$metblank,$gz_agents_img,$gz_member_force;
	...
	if($gz_member_use==2)$access_sql= " and access<=$metinfo_member_type";
	$numname = ' id,title,description,class1,class2,class3,updatetime,addtime,filename,access,top_ok,hits,issue,com_ok,no_order,';
	$listitem['news']=array(0=>$numname.'img_ok,imgurls,content,imgurl,links',1=>$gz_news,2=>'shownews');
	$listitem['product']=array(0=>$numname.'new_ok,imgurls,content,imgurl,displayimg,links',1=>$gz_product,2=>'showproduct');
	$listitem['download']=array(0=>$numname.'downloadurl,filesize,content,downloadaccess',1=>$gz_download,2=>'showdownload');
	$listitem['img']=array(0=>$numname.'new_ok,imgurls,content,imgurl,displayimg,links',1=>$gz_img,2=>'showimg');
	$listitem['job']=array(0=>'*',1=>$gz_job,2=>'showjob');
	...
	if($mark){
		...
		$folderone=$db->get_one("SELECT * FROM $gz_column WHERE bigclass='$listnowid' and module ='{$class_list[$listnowid][module]}' and releclass!='0' and lang='$lang'");
		if($folderone){
	...
	while($list= $db->fetch_array($result)){
		...
		if($list['top_ok']==1){
			$list['top']="<img class='listtop' src='".$img_url."top.gif"."' alt='".$gz_alt."' />";
			$list['hot']="";
			$list['news']="";
		}else{
			$list['top']="";
			$list['hot']=($list['hits']>=$gz_hot)?"<img class='listhot' src='".$img_url."hot.gif"."' alt='".$gz_alt."' />":"";
			$list['news']=(((strtotime($m_now_date)-strtotime($list['updatetime']))/86400)<$gz_newsdays)?"<img class='listnew' src='".$img_url."news.gif"."' alt='".$gz_alt."' />":"";
		}
		...
		if(strstr($list['imgurls'], "http://")){
			...
			$list['imgurls']=($list['imgurls']<>"")?$list['imgurls']:$weburly.$gz_agents_img;
		}
		if(strstr($list['imgurl'], "http://")){
			$list['imgurl']=($list['imgurl']<>"")?$list['imgurl']:$list['imgurl'];
		}else{	
			$list['imgurl']=($list['imgurl']<>"")?$list['imgurl']:$weburly.$gz_agents_img;
		}
		if(($dataoptimize[$pagemark]['para'][$modulenunm] and $dataoptimize[$pagemark]['parameter'])||$para){
			...
			$query1 = "select * from $gz_plist where lang='$lang' and listid='$list[id]' and module='$modulenunm' order by id";
			...
			while($list1 = $db->fetch_array($result1)){
				...
				if(intval($metparaaccess)>0&&$gz_member_use){
					$paracode=authcode($list[$nowpara1], 'ENCODE', $gz_member_force);
				...
				if($metpara[$list1['paraid']]['type']==5){
					...
					$fltp=$fltp?'gz_annex_'.$fltp:'';
					$list[$nowpara1]="<a href='{$list1['info']}' {$metblank} class='gz_annex {$fltp}' title='{$list1['imgname']}'>{$list1['imgname']}</a>";
		...
		$filenamenow=$gz_htmpagename==2?($modulenunm==6?'job':$class_list[$list['class1']]['foldername']):($gz_htmpagename==1?date('Ymd',strtotime($list['addtime'])):$listitem[$module][2]);
		$htmname=($list['filename']<>"")?$filename."/".$list['filename']:$filename."/".$filenamenow.$list['id'];
		$panyid = $list['filename']!=''?$list['filename']:$list['id'];
		$gz_ahtmtype = $list['filename']<>""?$gz_chtmtype:$gz_htmtype;
		$phpname=$gz_pseudo?$filename."/".$panyid.'-'.$lang.'.html':$filename."/".$listitem[$module][2].".php?".$langmark."&id=".$list['id'];	
		if($list['links']){
			$list['url']=$list['links'];
		}else{
			$list['url']=$gz_pseudo?$phpname:($gz_webhtm?$htmname.$gz_ahtmtype:$phpname);
		}
		$list['updatetime'] = date($gz_listtime,strtotime($list['updatetime']));
		$list['img_x']=gz_imgxy(1,$module);
		$list['img_y']=gz_imgxy(2,$module);
		$relist[]=$list;
	}
	return $relist;
}
...
if($listtype=='img'){
 ...
 $listtext.=" ><img src=".$val[imgurls]." alt='".$val[title]."' width=".$gz_img_x." height=".$gz_img_y." /></a></span>";
...
if($index[link_ok]==1 || $linkpage){
  ...
  if($listtype=='img'){
     ...
     foreach($linkarray as $key=>$val){
      ...
      $linktext.="<li style=\"";
      if($inewww==4){
        	$linktext.='margin-right:0px;';
        	$inewww=0;
        }
        if($i>4) $linktext.='margin-top:20px;';
      $linktext.="\" ><a href='".$val[weburl]."' target='_blank'><img src='".$val[weblogo]."' alt='".$val[webname]."' /></a></li>\n";
	  }
   }else{
     ...
     foreach($linkarray as $key=>$val){
	  ...
	  if($display=='select'){
        ...
        $linktext.="><a href='".$val[weburl]."' target='_blank' title='".$val[info]."'>".$val[webname]."</a></li>\n";
		}
	  }
     }
...
//foot info
$methtml_foot="<ul>\n";
if($gz_footright<>"" or $gz_footstat<>"")$methtml_foot.="<li>".$gz_footright." ".$gz_footstat."</li>\n";
if($gz_footaddress<>"")$methtml_foot.="<li>".$gz_footaddress."</li>\n";
if($gz_foottel<>"")$methtml_foot.="<li>".$gz_foottel."</li>\n";
if($gz_footother<>"")$methtml_foot.="<li>".$gz_footother."</li>\n";
if($gz_foottext<>"")$methtml_foot.="<li>".$gz_foottext."</li>\n";
$methtml_foot.="</ul>\n";
//接口代码
if($_M['html_plugin']['foot_script'])$methtml_foot.="{$_M['html_plugin']['foot_script']}";
//结束
if($gz_jiathis_ok)$methtml_foot.=$gz_jiathis;

//online
function methtml_online(){
	global $gz_online_type,$navurl,$gz_onlineleft_left,$gz_onlineright_right,$gz_onlineleft_top,$gz_onlineright_top,$lang,$gz_stat_js;
	$metinfo='';
	if($gz_stat_js)$metinfo.=$gz_stat_js."\n";
	if($gz_online_type!=3){
		...
		$metinfo.="<script src='{$navurl}public/js/online.js?t={$gz_online_type}&u={$navurl}&x={$onlinex}&y={$onliney}&lang={$lang}' type='text/javascript' id='metonlie_js'></script>";
	}
	return $metinfo;
}

function methtml_hits($module,$mobile){
global $news,$product,$img,$download,$job,$lang_Hits,$lang_Printing,$lang_Printing,$lang_UpdateTime,$lang_Close,$gz_tools_ok,$gz_tools_code,$metinfover;
global $gz_pageclick,$gz_pagetime,$gz_pageprint,$gz_pageclose;
	$listnow=$$module;
	if($module=='job')$listnow[updatetime]=$listnow[addtime];
	$metinfo.="<div class='metjiathis'>{$gz_tools_code}</div>";
	if($metinfover == 'v1'){
		if($module!='job' && $gz_pageclick)$metinfo.=$lang_Hits."：<span class='metClicks' data-metClicks=".$module."|".$listnow[id]."></span>";
		if($gz_pagetime)$metinfo.='　'.$lang_UpdateTime.'：'.$listnow['updatetime'];
		if($gz_pageprint)$metinfo.='　【<a href="#" class="metPrinting">'.$lang_Printing.'</a>】';
		if($gz_pageclose)$metinfo.='　【<a href="#" class="metClose">'.$lang_Close.'</a>】';
	}else{
...
global $gz_member_use,$navurl,$lang,$lang_memberName,$gz_member_login,$lang_memberPs,$gz_memberlogin_code,$lang_memberImgCode,$lang_memberTip1,$lang_register,$lang_memberGo,$lang_memberIndex2,$metinfo_member_name,$lang_memberIndex1,$member_index_url,$lang_memberIndex10,$member_registerurl;
global $lang_memberPassword,$lang_memberRegisterName,$metinfo_member_id;
$metinfo_member_id = get_gz_cookie('metinfo_member_id');
...
function memberlist(){
    global $lang,$gz_pseudo,$weburly,$lang_memberIndex3,$lang_memberIndex4,$lang_memberIndex5,$lang_memberIndex6,$lang_memberIndex7,$lang_memberIndex10;
	...
	foreach($list as $key=>$val){
	    $url=$gz_pseudo?$val[url].'-'.$lang.'.html':$val[url].".php?lang=$lang";
