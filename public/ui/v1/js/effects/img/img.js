define(function(require, exports, module) {
	...
	if(MetpageType==3){//详情页
		...
		$('#showimg .gz_slide_list').width($('#showimg .gz_slide_list img').attr("width"));
		
		require('effects/img/css/imgshow.css');
		require('effects/img/jquery.exposure.min');
		var width  = $(".gz_slide_box").attr("data-sidewidth"),
			height = $(".gz_slide_box").attr("data-sideheight");
		$(".gz_slide_box").width(width);
		$(".gz_slide_box .left,.gz_slide_box .right").css({"top":'0px',"opacity":"0","height":height+'px'});
		$(".gz_slide_box .left,.gz_slide_box .right").hover(function(){
			$(".gz_slide_box .left,.gz_slide_box .right").stop(true,true);
			$(this).fadeTo("slow", 1);
		},function(){
			$(".gz_slide_box .left,.gz_slide_box .right").stop(true,true);
			$(this).fadeTo("slow", 0);
		});
		$('.gz_slide_list ul').exposure({carouselControls : true,
			imageControls : true,
			pageSize : 5,
		...
		$(".exposureThumbs img,.gz_slide_box .left,.gz_slide_box .right,#exposure").click(function(){
