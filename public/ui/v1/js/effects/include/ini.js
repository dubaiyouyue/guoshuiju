		...
		if(metPage.length>0){
			...
			function mPage(mett,metv){
				...
				if(metPageNums){
					...
					if(metPageNums<0){
						var pageUrl = gz_weburl + folder + "/" +  PageStr[0] + "1" + PageStr[1];
						window.location.href = pageUrl;
					}else if(metPageNums>PageStr[2]){
						var pageUrl = gz_weburl + folder + "/" +  PageStr[0] + PageStr[2] + PageStr[1];
						window.location.href = pageUrl;
					}else{
						var pageUrl = gz_weburl + folder + "/" + PageStr[0] + metPageNums + PageStr[1];
						window.location.href = pageUrl;
					}
