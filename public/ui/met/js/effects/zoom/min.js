define(function(require, exports, module) {
	...
	var labelBut = "<div class='gz_ButPrev'></div><div class='gz_ButNext'></div>";
	...
	var _metSpan = _metTab.find('span'),_left = _metTab.find(".gz_ButPrev"),_right = _metTab.find(".gz_ButNext");
	...
	_left.hover(function () {
					_metTab.find('.gz_ButPrev').css('background-position','left 18px');
					if(_if){
					...
					_metTab.find('.gz_ButPrev').css('background-position','left -42px');
					clearInterval(_time)
			 }); 
	_right.hover(function () {
					_metTab.find('.gz_ButNext').css('background-position','right 18px');
					if(_if){
					...
					_metTab.find('.gz_ButNext').css('background-position','right -42px');
					clearInterval(_time)
			 });
