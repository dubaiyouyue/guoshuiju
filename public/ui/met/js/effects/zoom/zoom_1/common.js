...
JSMENU['drag'] = [];
JSMENU['layer'] = 0;
JSMENU['zIndex'] = {'win':200,'menu':500,'dialog':400,'prompt':500};
JSMENU['float'] = '';
		...
		if(aj.XMLHttpRequest.readyState == 4 && aj.XMLHttpRequest.status == 200) {
			...
			if(aj.recvType == 'HTML') {
				...
				if(!aj.XMLHttpRequest.responseXML || !aj.XMLHttpRequest.responseXML.lastChild || aj.XMLHttpRequest.responseXML.lastChild.localName == 'parsererror') {
					aj.resultHandle('<a href="' + aj.targetUrl + '" target="_blank" style="color:red">'+gz_error_d+'</a>' , aj);
				} else {
...
function ajaxpost(formid, showid, waitid, showidclass, submitbtn, recall) {
	...
	var handleResult = function() {
		...
		showloading('none');
		try {
			s = $id(ajaxframeid).contentWindow.document.XMLDocument.text;
		} catch(e) {
			try {
				s = $id(ajaxframeid).contentWindow.document.documentElement.firstChild.wholeText;
			} catch(e) {
				try {
					s = $id(ajaxframeid).contentWindow.document.documentElement.firstChild.nodeValue;
				} catch(e) {
					s = gz_error_d;
				}
			}
		...
		if(showidclass) {
			if(showidclass != 'onerror') {
				$id(showid).className = showidclass;
			} else {
				showError(s);
				ajaxerror = true;
			}
		...
		if(!evaled && (typeof ajaxerror == 'undefined' || !ajaxerror)) {
			ajaxinnerhtml($id(showid), s);
		}
		ajaxerror = null;
		if($id(formid)) $id(formid).target = formtarget;
		...
		ajaxframe.loading = 0;
		if(!BROWSER.firefox) {
			$id('append_parent').removeChild(ajaxframe.parentNode);
		} else {
			setTimeout(
				function(){
					$id('append_parent').removeChild(ajaxframe.parentNode);
...
function showloading(display, waiting) {
	...
	var waiting = waiting ? waiting : gz_loading_d;
...
function showWindow(k, url, mode, cache, menuv) {
	...
	if(disallowfloat && disallowfloat.indexOf(k) != -1) {
		if(BROWSER.ie) url += (url.indexOf('?') != -1 ?  '&' : '?') + 'referer=' + escape(IMGDIRaa);
		IMGDIRaa = url;
		doane();
		return;
	}
	...
	var fetchContent = function() {
		if(mode == 'get') {
			...
			if(BROWSER.ie &&  url.indexOf('referer=') < 0) {
				url = url + '&referer=' + encodeURIComponent(IMGDIRaa);
			}
			ajaxget(url, 'fwin_content_' + k, null, '', '', function() {initMenu();show();});
		} else if(mode == 'post') {
			menuObj.act = $id(url).action;
			ajaxpost(url, 'fwin_content_' + k, '', '', '', function() {initMenu();show();});
		}
		if(parseInt(BROWSER.ie) != 6) {
			loadingst = setTimeout(function() {showDialog('', 'info', '<img src="'+ IMGDIR + '/loading.gif"> '+gz_loading_d)}, 500);
		}
	};
	...
	if(!menuObj) {
		...
		evt = ' style="cursor:move" onmousedown="dragMenu($id(\'' + menuid + '\'), event, 1)" ondblclick="hideWindow(\'' + k + '\')"';
		if(!BROWSER.ie) {
		...
		if(mode == 'html') {
			$id('fwin_content_' + k).innerHTML = url;
			initMenu();
			show();
		} else {
			fetchContent();
		}
	} else if((mode == 'get' && (url != menuObj.url || cache != 1)) || (mode == 'post' && $id(url).action != menuObj.act)) {
		fetchContent();
	} else {
		show();
	}
...
function thumbImg(obj, method) {
	...
	if(zw < 2) {
		...
		setTimeout("thumbImg($id('" + obj.id + "'), " + method + ")", 100);
		return;
	}
	zr = zw / zh;
	method = !method ? 0 : 1;
