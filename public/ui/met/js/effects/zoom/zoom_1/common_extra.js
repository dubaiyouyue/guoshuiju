...
//alert(455);
	...
	if(!menu && !pn) {
		...
		menu.id = menuid;
		if(cover) {
			menu.innerHTML = '<div class="zoominner" id="' + menuid + '_zoomlayer" style="display:none"><p><span class="y"><a id="' + menuid + '_imglink" class="imglink" target="_blank" title="'+gz_blank_d+'">'+gz_blank_d+'</a><a id="' + menuid + '_adjust" href="javascipt:;" class="imgadjust" title="'+gz_full_d+'">'+gz_full_d+'</a>' +
				'<a href="javascript:;" onclick="hideMenu()" class="imgclose" title="'+gz_close_d+'">'+gz_close_d+'</a></span>'+gz_zoom_d+'</p>' +
				'<div class="zimg_p" id="' + menuid + '_picpage"></div><div class="hm" id="' + menuid + '_img"></div></div>';
		} else {
			menu.innerHTML = '<div class="popupmenu_popup" id="' + menuid + '_zoomlayer" style="width:auto"><span class="right y"><a href="javascript:;" onclick="hideMenu()" class="flbc" style="width:20px;margin:0 0 2px 0">'+gz_close_d+'</a></span>'+gz_zoom_d+'<div class="zimg_p" id="' + menuid + '_picpage"></div><div class="hm" id="' + menuid + '_img"></div></div>';
		}
	...
	// alert(4545445);
		...
		if(authorlength > 1) {
			...
			if(authorcurrent !== '') {
				...
				picpage += ' <div id="zimg_prev" onmouseover="dragMenuDisabled=true; onstate(this,\'133px\');" onmouseout="dragMenuDisabled=false; onstate(this,\'10000px\');" onclick="_zoom_page(\'' + paid + '\', ' + (showexif ? 1 : 0) + ')" class="zimg_prev"><strong>'+gz_zimg_prev+'</strong></div> ';
				paid = authorcurrent < authorlength - 1 ? authorimgs[authorcurrent + 1] : authorimgs[0];
				picpage += ' <div id="zimg_next" onmouseover="dragMenuDisabled=true; onstate(this,\'133px\');" onmouseout="dragMenuDisabled=false; onstate(this,\'10000px\');" onclick="_zoom_page(\'' + paid + '\', ' + (showexif ? 1 : 0) + ')" class="zimg_next"><strong>'+gz_zimg_next+'</strong></div> ';
			}
			if(picpage) {
