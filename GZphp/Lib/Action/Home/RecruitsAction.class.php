...
class RecruitsAction extends BaseAction
{
    public function index()
    {
		...
		$blist=$this->bannerlists(469);
		$getinfolamu=$this->get_column_info(469);$this->assign('getinfolamu',$getinfolamu);
		  $lanmu=M('column')->where('bigclass=469')->order('no_order asc,id asc')->select();
       ...
       $p=$_GET['p']+0;
		...
		if($namemark=='new') $m=4;
		$m=3;
		$m=$lan['listnumber']?$lan['listnumber']:3;
		$cclslss='class2';
		if($cid==469) $cclslss='class1';
      ...
      //	print_r($list);exit();
        ...
        if($ajax=='ajax') $this->display('./GZphp/Tpl/Home/Default/Recruits_ajax.html');
	...
	public function ajax(){
		...
		if($cid==469) $list=$this->new_list('class1',$cid,$p,$m);
