...
class BrandAction extends BaseAction
{
    public function index(){
		...
		global $foldername;
		global $namemark_tpl;
		$blist=$this->bannerlists(462);
		$getinfolamu=$this->get_column_info(462);$this->assign('getinfolamu',$getinfolamu);
			//print_r($blist);exit();
        $lanmu=M('column')->where('bigclass=462')->order('id asc')->select();
		  $news=M('news')->where("class1=462")->order('id asc')->select();
		...
		//echo $p;exit;
		$m=5;
		$m=$lan['listnumber']?$lan['listnumber']:2000;
		if($namemark=='new') $m=5;
		
		$cclslss='class2';
		if($cid==462) $cclslss='class1';
		
		$list=$this->new_list($cclslss,$cid,$p,$m,'','',$hyid);
		//if(empty($list)) exit('no data');
		$lanmumm=$this->lanmu_list('bigclass',462,$p,$m);
		...
		if($ajax=='ok') $this->display('./GZphp/Tpl/Home/Default/Product_ajax.html');
