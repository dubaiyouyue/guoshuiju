...
class TaxAction extends BaseAction
{
    public function index()
    {
		...
		$blist=$this->bannerlists($cid);
		$getinfolamu=$this->get_column_info($cid);$this->assign('getinfolamu',$getinfolamu);
		  $lanmu=M('column')->where('bigclass='.$cid)->order('no_order asc,id asc')->select();
       ...
       $p=$_GET['p']+0;
		...
		if($namemark=='new') $m=4;
		$m=3;
		$m=$lan['listnumber']?$lan['listnumber']:11;
		$cclslss='class1';
      ...
      //	print_r($list);exit();
        ...
        if($ajax=='ajax') $this->display('./GZphp/Tpl/Home/Default/Trends_ajax.html');
