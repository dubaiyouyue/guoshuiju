...
class ServicsAction extends BaseAction
{
    public function index()
    {
		...
		global $foldername;
		global $namemark_tpl;
		$blist=$this->bannerlists(464);
		$getinfolamu=$this->get_column_info(464);$this->assign('getinfolamu',$getinfolamu);
		//dump($getinfolamu);exit;
		   $lanmu=M('column')->where('bigclass=464')->order('no_order asc,id asc')->select();
		...
		$this->assign('lan',$lan);
        if($ssshowss=='show') $this->display('./GZphp/Tpl/Home/Default/Servics_sss.html');
