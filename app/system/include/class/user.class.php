...
class user{	
	...
	public function register($username, $password, $email, $tel, $info, $valid, $groupid, $source,$jihuo){
		global $_M;
		return $this->insert_uesr_sql($username, $password, $email, $tel, $valid, $groupid, $source,$jihuo);
		if($userid){
	...
	public function insert_uesr_sql($username, $password, $email, $tel, $valid, $groupid = '', $source = '',$jihuo, $register_time = '', $register_ip = '', $login_time = '', $login_ip = '', $login_count = ''){
		global $_M;
	...
	/*编辑信息*/
	public function editor_uesr($userid, $email, $tel, $valid, $groupid,$xm,$password,$jihuo){
		global $_M;
		if(!$userid){
			return false;
		}
	...
	public function login_out() {
		global $_M;
		$_M['user'] = array();
		gz_setcookie("gz_auth", '', -3600);
		gz_setcookie("gz_key", '', -3600);
	}
	...
	public function get_user_by_username_tel_sql($tel) {
		global $_M;
	...
	public function setauth($username, $password){
		...
		gz_setcookie("acc_auth",$private_auth, 0);
		gz_setcookie("acc_key",$private_key, 0);
	}
	...
	public function logout(){
		global $_M;
		gz_setcookie("acc_auth",'');
		gz_setcookie("acc_key",'');
		$this->set_m('');
	}
}
