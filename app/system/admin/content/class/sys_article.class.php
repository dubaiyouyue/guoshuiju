...
class sys_article extends module {
	...
	public function update_list($list,$id){
		...
		$list['imgurl']=str_ireplace('//','/',$list['imgurl']);
		$list['imgurl']=str_ireplace('../','/',$list['imgurl']);
		$list['imgurls']=str_ireplace('//','/',$list['imgurls']);
		$list['imgurls']=str_ireplace('../','/',$list['imgurls']);
	...
	public function update_list_sql($list,$id){
		...
		if(!$list['tvlinks']){
			$querytv = "SELECT tvlinks  FROM {$this->tablename} WHERE id='{$id}'";
			$listowntv = DB::get_one($querytv); 

			$list['tvlinks']=$listowntv['tvlinks'];
		}
		...
		$query = "UPDATE {$this->tablename} SET
			...
			links              = '{$list['links']}',
			top_ok             = '{$list['top_ok']}',
			tvlinks            = '{$list['tvlinks']}',
			exhibitiontime            = '{$list['exhibitiontime']}',
			exhibitionnum            = '{$list['exhibitionnum']}',
			dezxcsption            = '{$list['dezxcsption']}',
			gz_headeryjhhyy            = '{$list['gz_headeryjhhyy']}',
			exhibitionurl            = '{$list['exhibitionurl']}',
