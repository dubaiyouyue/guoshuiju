...
class article_admin extends admin {
	...
	function doadd() {
		...
		$list['issue'] = get_gz_cookie('metinfo_admin_name');
		$a = 'doaddsave';
	...
	function doeditor() {
		...
		$list['updatetime'] = date("Y-m-d H:i:s");
		$list['issue'] = $list['issue'] ? $list['issue'] : get_gz_cookie('metinfo_admin_name');
		$a = 'doeditorsave';
	...
	function dojson_list(){
		...
		$gz_class = $this->moduleclass->column(2,$this->module);
		$order = $this->moduleclass->list_order($gz_class[$classnow]['list_order']);
		...
		foreach($userlist as $key=>$val){
			...
			$list[] = $val['id'];
			$list[] = "<div class=\"ui-table-a\"><a href=\"{$_M[url][own_form]}a=doeditor&id={$val['id']}&select_class1={$class1}&select_class2={$class2}&select_class3={$class3}\">{$val['title']}</a></div>";
			$list[] = $val['hits'];
			$list[] = $val['updatetime'];
			$list[] = $val['state'];
