...
class sys_user extends user {	
	public function json_user_list($orderf){
		global $_M;
		...
		$search.= $keyword?"and (xm like '%{$keyword}%' || bm like '%{$keyword}%' || tel like '%{$keyword}%')":'';  
		
		$table = load::sys_class('tabledata', 'new');
		$order = "id DESC";
		if($orderf) $order = $orderf;
		...
		foreach($list as $id){
			if($id){
				if(is_number($id)){
					...
					DB::query($query);
					$query = "DELETE FROM {$_M['table']['user_other']} WHERE gz_uid='{$id}'";
					DB::query($query);
