...
global $checksum,$gz_host,$gz_file;
	if($typedel==2){
		delcodeb($checksum);
	}
}
function dl_error($alert,$type,$olid,$ver,$addr,$action,$num=0){
global $checksum,$gz_host,$gz_file,$db,$url_array,$lang_retested,$lang_redownload,$lang_updaterr21,$lang_updaterr22;
	if($action!='error'){
		if($action=='dl'){
			$num=$num-1;
	...
	if($type==1){
		echo "<a href=\"http://$gz_host/dl/olupdate.php\" onclick=\"return olupdate('cms','new','test');\">{$lang_retested}</a>"; 
	}
	if($type==2){
		if($addr)deldir("../app/$addr/");
		$query="select * from $gz_app where no=$olid and download=1";
		$appver=$db->get_one($query);
		$verold=is_array($appver)?$appver['ver']:0;
		echo "<a href='http://$gz_host/dl/app.php' onclick=\"return olupdate('$olid','$verold','testc');\">{$lang_redownload}</a>"; 
	}
...
if($action=="check"){
	...
	if($type==2){
		$query="select * from $gz_app where no='$olid' and download='0'";
		...
		if($results[2]=='noaddr'){
			delcodeb($return['md5']);
			$results[1]="<a href=\"http://$gz_host/dl/app.php\" onclick=\"return olupdate('$olid','0','testc');\"><img src=\"../../templates/met/images/dwn.png\"><p>{$lang_usertype1}{$lang_appinstall}</p></a><script type=\"text/javascript\">alert('$results[1]')</script>";
	...
	if($type==1){
		$gz_file='/dl/olupdate.php';
		$post_data = array('test'=>$test,'ver'=>$ver);
		$result=curl_post($post_data,30);
		$results=explode('|',$result);
		$query="update $gz_config set value='$results[2]' where name='gz_newcmsv'";
		$db->query($query);
		echo $result;
	}
}
	...
	if($backup==1){
		...
		do{
			...
			if(trim($sqldump)){
				...
				$sqldump = "#resonance.com.cn Created {$version} \n#{$gz_weburl}\n#{$tablepre}\n# --------------------------------------------------------\n\n\n".$sqldump;
	...
	if($type==1){
		$db->query("update $gz_config set value='$ver' where name='metcms_v'");
		$authinfo=$db->get_one("SELECT * FROM $gz_otherinfo where id=1");
		$gz_file='/dl/record_dl.php';
		$post_data = array('cmd'=>'sys','url'=>$gz_weburl,'code'=>$authinfo['authcode'],'key'=>$authinfo['authpass'],'ver'=>$ver);
		...
		$query="select * from $gz_app where no=$olid and download=1";
		if($db->get_one($query)){
			$query="update $gz_app set name='$app[name]',ver='$app[ver]',img='$app[img]',info='$app[info]',file='$app[file]',power='$app[power]',sys='$app[sys]',site='$app[site]',url='$app[url]' where no='$app[no]' and download=1";
			$db->query($query);
		}
		else{
			$query="insert into $gz_app set name='$app[name]',no='$app[no]',ver='$app[ver]',img='$app[img]',info='$app[info]',file='$app[file]',power='$app[power]',sys='$app[sys]',site='$app[site]',url='$app[url]',download=1";	
			$db->query($query);
		}
		$query="select * from $gz_admin_table where usertype=3";
		...
		while($list=$db->fetch_array($result)){
			...
			if(stripos($list[admin_type_tmp],'-s142-')!==false){
				...
				$query="update $gz_admin_table set admin_type='$list[admin_type]' where id='$list[id]'";
				$db->query($query);
			}
		}
		$gz_file='/dl/record_dl.php';
