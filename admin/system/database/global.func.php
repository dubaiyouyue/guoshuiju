...
{
	global $db,$tablepre,$gz_visit_day,$gz_visit_detail,$gz_admin_table;
    $split = sql_split($sql);
	$sqls  = $split['sql'];
	$info  = $split['info'];
	$infos=explode('#',$info);
	...
	if(is_array($sqls))
    {
		...
		{
			if($replace){
				$sql=str_replace('gz_',$tablepre,$sql);
				$sql=str_replace('metconfig_','gz_',$sql);
			}
			if($dosubmit1 == '1'){
				$sql = preg_replace(array('/INSERT INTO '.$gz_admin_table.'/','/DROP TABLE IF EXISTS '.$gz_admin_table.'/','/CREATE TABLE `'.$gz_admin_table.'`/'),array('INSERT INTO test_admin_table1','DROP TABLE IF EXISTS test_admin_table1','CREATE TABLE `test_admin_table1`'),$sql);
			}
			...
			if($sqlre2==1){
				if(!preg_match('/^INSERT INTO (('.$gz_visit_day.')|('.$gz_visit_detail.'.))/',$sql)){
...
global $gz_tablename;
	$mettables=explode('|',$gz_tablename);
	$i=0;
