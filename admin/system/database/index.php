...
if($action=='allfile'){
	...
	if(trim($sqldump)){
		...
		$sqlzip='../../databack/sql/'.$gz_agents_backup.'_'.$zipname.'.zip';
		...
		$sqlzip='../../databack/web/'.$gz_agents_backup.'_web_'.$con_db_name.'_'.date('YmdHis',time()).'_'.gz_rand(6).'.zip';
		$zipfile="../../../";
		$archive = new PclZip($sqlzip);
	...
	$sqlzip='../../databack/upload/'.$gz_agents_backup.'_upload_'.date('YmdHis',time()).'.zip';
	$zipfile="../../../upload";
	$archive = new PclZip($sqlzip);
	...
	$sqlzip='../../databack/config/'.$gz_agents_backup.'_config_'.date('YmdHis',time()).'.zip';
