...
if($action=='delete'){
	...
	foreach($sqlfiles as $id=>$sqlfile){
		...
		if(stripos($sqlfile,$prefix)!==false){
			if(fileext($sqlfile)=='sql'){
				...
				@unlink('../../databack/sql/'.$gz_agents_backup.'_'.$filenamearray[0].".zip");
			}
		}
	}
	metsave($rurls,'',$depth);
}else if($action=='daoru'){
	$query="select admin_op from $gz_admin_table where admin_id='{$metinfo_admin_name}'";
	...
	if(file_exists($filepath)){
		...
		if($action == 'daoru'){
			if(stristr($sql,'INSERT INTO gz_admin_table')){
				echo "<script>
	...
	if(file_exists($filepath)){
		...
		$query="select * from $gz_column where ((module<=5 and module>0) or (module=8)) and (classtype=1 or releclass!=0)";
	...
	foreach($infos as $key=>$val){
		...
		$str = @fgets($fp);
		@fclose($fp);
		$infos[$key]['ver']=trim(str_replace('#resonance.com.cn Created version:','',$str));
	}
	foreach($infos as $key=>$val){
	...
	if($gz_agents_type>1){
		$lang_dataexplain2=str_replace('met',$gz_agents_backup,$lang_dataexplain2);
	}
