...
function gz_ckeditor(depthm, name, type) {
...
function emailtest(met) {
	var usename = $("input[name='gz_fd_usename']").val();
	var smtp = $("input[name='gz_fd_smtp']").val();
	var password = $("input[name='gz_fd_password']").val();
	var fromname = $("input[name='gz_fd_fromname']").val();
...
function okonlineqq(type) {
	...
	$('#met' + type + 'img').attr('src', '../../../public/images/' + type + '/' + type + '_' + $("input[name='gz_" + type + "_type']:checked").val() + hz);
...
function olupdate(id, ver, action) {
	...
	var url_r = $('#gz_url_r').val() + 'olid=' + id;
	var url_h = $('#gz_url_h').val() + 'olid=' + id + '&ver=' + ver + '&addr=' + addr;
	if ($('#gz_url_p').val() == 1) {
		url_r += '&gz_code=' + $('#gz_code').val() + '&gz_key=' + $('#gz_key').val();
	}
	if ($('#gz_url_p').val() == 1 && !(action == "test" || action == "testc")) {
...
function olflie(id, ver, action, numnow) {
	...
	var url_h = $('#gz_url_h').val() + 'olid=' + id + '&ver=' + ver + '&addr=' + addr + '&numnow=' + numnow + '&checksum=' + checksum + '&action=dl';
	$.ajax({
		url: url_h,
		type: 'POST',
		data: '',
...
function CheckAllx(my, fm,name) {
	...
	sid = sid.size() > 0 ? sid: $("input[name^='gz_flash_']");
...
function gz_modify(my, form, type) {
