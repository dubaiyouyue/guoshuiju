...
if($action=="linkopen"){
$gz_addlinkopen=$gz_addlinkopen;
$langp=$lang;
$metcms[$langp]['gz_addlinkopen']=$gz_addlinkopen;
...
okinfo('../link/index.php?lang='.$lang);
    ...
    if($search == "detail_search") {	
        ...
        $total_count = $db->counter($gz_link, "$serch_sql", "*");
    }else{
        $total_count = $db->counter($gz_link, "$serch_sql", "*");
    }
    ...
    $query = "SELECT * FROM $gz_link $serch_sql $order_sql LIMIT $from_record, $list_num";
...
$gz_weburl=substr($gz_weburl, 0, -1);
