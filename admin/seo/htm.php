...
if($action=="all"){
	...
	if($htmpack){
		...
		$gz_htmpack_url='../databack/htmpack/';
		if(is_dir($gz_htmpack_url))deldir($gz_htmpack_url);
		$lista = $gz_htmpack_url.'templates/'.$gz_skin_user.'/images/';
		metnew_dir($lista);
		$lista = $gz_htmpack_url.'public/';
		metnew_dir($lista);
		
		$dira=$gz_htmpack_url.'templates/'.$gz_skin_user.'/images/';
		$dirb="../../templates/".$gz_skin_user.'/images';
		if(file_exists($dirb)){
			xCopy($dirb,$dira,1);
		}
		
		$dira=$gz_htmpack_url.'templates/'.$gz_skin_user.'/js/';
		$dirb="../../templates/".$gz_skin_user.'/js';
		if(file_exists($dirb)){
			xCopy($dirb,$dira,1);
		}
		
		$dira=$gz_htmpack_url.'templates/'.$gz_skin_user.'/css/';
		$dirb="../../templates/".$gz_skin_user.'/css';
		if(file_exists($dirb)){
			@xCopy($dirb,$dira,1);
		}
		
		$dira=$gz_htmpack_url.'upload/';
		$dirb="../../upload/";
		xCopy($dirb,$dira,1);
		
		$dira=$gz_htmpack_url.'public/banner/';
		$dirb="../../public/banner/";
		xCopy($dirb,$dira,1);
		
		$dira=$gz_htmpack_url.'public/images/';
		$dirb="../../public/images/";
		xCopy($dirb,$dira,1);
		
		$dira=$gz_htmpack_url.'public/js/';
		$dirb="../../public/js/";
		xCopy($dirb,$dira,1);
		
		$dira=$gz_htmpack_url.'public/css/';
		$dirb="../../public/css/";
		xCopy($dirb,$dira,1);
		
		$dira=$gz_htmpack_url.'public/ui/';
		$dirb="../../public/ui/";
		xCopy($dirb,$dira,1);
		
		$dira=$gz_htmpack_url.'favicon.ico';
		$dirb="../../favicon.ico";
		copy($dirb,$dira);
	}
	$methtm[]= indexhtm(1,$htmpack);
	//module 1
	foreach($gz_classindex[1] as $key=>$val){
	...
	foreach($gz_classindex[2] as $key=>$val){
		...
		$query="select * from $gz_news where class1='$val[id]' and lang='$lang' and (recycle='0' or recycle='-1')";
	 ...
	 foreach($gz_classindex[3] as $key=>$val){
		...
		$query="select * from $gz_product where class1='$val[id]' and lang='$lang' and (recycle='0' or recycle='-1')";
		$result= $db->query($query);
		while($list = $db->fetch_array($result)){
	 ...
	 //module 4
	 foreach($gz_classindex[4] as $key=>$val){
		$methtmx.=classhtm($val[id],0,0,1,0,$htmpack).'$|$';
		if($val['releclass']){
			foreach($gz_class3[$val[id]] as $key=>$val3){
				$methtmx.=classhtm($val[id],$val3[id],0,1,2,$htmpack).'$|$';
		...
		$query="select * from $gz_download where class1='$val[id]' and lang='$lang' and (recycle='0' or recycle='-1')";
		$result= $db->query($query);
		 while($list = $db->fetch_array($result)){
	 ...
	 //module 5
	 foreach($gz_classindex[5] as $key=>$val){
		$methtmx.=classhtm($val[id],0,0,1,0,$htmpack).'$|$';
		if($val['releclass']){
			foreach($gz_class3[$val[id]] as $key=>$val3){
				$methtmx.=classhtm($val[id],$val3[id],0,1,2,$htmpack).'$|$';
		...
		$query="select * from $gz_img where class1='$val[id]' and lang='$lang' and (recycle='0' or recycle='-1')";
		$result= $db->query($query);
		 while($list = $db->fetch_array($result)){
	 ...
	 //module 6
	 foreach($gz_classindex[6] as $key=>$val){
		$methtmx.=classhtm($val[id],0,0,1,0,$htmpack).'$|$';
		$methtm[]=onepagehtm('job','cv',1,$htmpack); 
		$query="select * from $gz_job where lang='$lang'";
		$result= $db->query($query);
		 while($list = $db->fetch_array($result)){
	...
	if($module==1){
		$folder=$gz_class[$class1];
		$methtm[]=showhtm($class1,1);
		if($gz_class[$class1]['releclass']){
			foreach($gz_class3[$class1] as $key=>$val1){
				if($val1[isshow])$methtm[]=showhtm($val1[id],1);
			}
		}
		else{
			foreach($gz_class22[$class1] as $key=>$val){
				if($val[isshow])$methtm[]=showhtm($val[id],1);
				foreach($gz_class3[$val[id]] as $key=>$val1){
					if($val1[isshow])$methtm[]=showhtm($val1[id],1);
				}
			}
		}
	...
	if($module>=2 && $module<=5){
	    if($listall=="all"){
			if($gz_class[$class1]['releclass']){
				...
				foreach($gz_class3[$class1] as $key=>$val3){
			...
			$result= $db->query($query);
			while($list = $db->fetch_array($result)){
				$methtm[]=contenthtm($class1,$list[id],$filename,$list[filename],1,$gz_class[$class1][foldername],$list[addtime]);
			}
	...
	if($class1=='login'&&$gz_member_use!=0){
