...
$query="select * from $gz_column where lang='$lang' and if_in='0' order by no_order";
...
switch($mtype){
	case 1:
		$gz_module_type=$lang_flashMode1;
	break;
	case 2:
		$gz_module_type=$lang_flashMode2;
	break;
	case 3:
		$gz_module_type=$lang_setflashMode3;
	break;
}
if($module==10000||$module==10001){
	$columnid=$module==10000?array('name'=>$lang_flashGlobal):array('name'=>$lang_indexhome);
}else{
	$columnid=$db->get_one("select * from {$gz_column} where id='{$module}' and lang='{$lang}'");
}
$i=1;
...
$gz_flash_type[$gz_flasharray[10000][type]]="checked='checked'";
$style1=$gz_flasharray[10000][type]==1?"style='display:block;'":"style='display:none;'";
