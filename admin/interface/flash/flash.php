...
if($module<>""){
	$dule = $gz_flasharray[$module]['type']==2?"(flash_path !='' and module = 'metinfo')":"(img_path !='' and module = 'metinfo')";
	$where ="where lang='$lang' and wap_ok='0' and (module like '%,{$module},%' or {$dule}) order by no_order";
	$module1[$module]='selected';
}
$total_count = $db->counter($gz_flash, "$where", "*");
$rowset = new Pager($total_count,$list_num,$page);
$from_record = $rowset->_offset();
$query = "SELECT * FROM {$gz_flash} {$where} LIMIT {$from_record}, {$list_num}";
if($module<>"" && !$gz_flasharray[$module]['type'])$query='';
		...
		for($i=0;$i<count($lmod);$i++){
			if($lmod[$i]!=''){
				if($lmod[$i]==10001){
					...
					$columnids=$db->get_one("select * from {$gz_column} where id='{$lmod[$i]}' and lang='{$lang}'");
...
$query1="select * from $gz_column where if_in='0' and lang='$lang' order by no_order";
