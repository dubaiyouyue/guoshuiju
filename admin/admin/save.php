...
if($action=="add"){
	if(($admincp_ok['admin_group']=='10000'||($admincp_ok['admin_group']=='3'&&$admincp_ok['admin_group']>$admin_group))&&$admin_group!='10000'){
		$admin_if=$db->get_one("SELECT * FROM $gz_admin_table WHERE admin_id='$useid'");
		if($admin_if)metsave('-1',$lang_loginUserMudb1);
		$pass1=md5($pass1);
		$query = "INSERT INTO $gz_admin_table SET
			admin_id           = '$useid',
			admin_pass         = '$pass1',
			admin_name         = '$name',
			admin_sex          = '$sex',
...
if($action=="editor"){
	if($edtp==2){
		if($admincp_ok['admin_group']=='10000'){
			$query="update $gz_admin_table set admin_id='$change_admin_id' where admin_group='10000' and admin_id='admin'";
			$db->query($query);
		}
		metsave('../admin/index.php?lang='.$lang.'&anyid='.$anyid);
	}else{
		$query = "select * from $gz_admin_table where id='$id'";
		...
		if(($admincp_ok['admin_group']=='10000')||($admincp_ok['admin_group']=='3'&&$admincp_ok['admin_group']>$modify['admin_group'])||$modify['id']==$admincp_ok['id']){
			$query = "update $gz_admin_table SET admin_ok = '$admin_ok'";
			if($edtp!=1){
