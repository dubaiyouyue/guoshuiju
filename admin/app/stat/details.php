...
$et=$et+86300;
if($setip!=null){
	$total_count = $db->counter($gz_visit_day, " acctime >='{$st}' and acctime<='{$et}' and ip='$setip'", "*");
}else{
	$total_count = $db->counter($gz_visit_day, " acctime >='{$st}' and acctime<='{$et}'", "*");
}
$list_num = 15;
...
if($setip!=null){
	$query="select * from {$gz_visit_day} WHERE acctime >='{$st}' and acctime<='{$et}' and ip='$setip' order by acctime desc LIMIT {$from_record},{$list_num}";
}else{
	$query="select * from {$gz_visit_day} WHERE acctime >='{$st}' and acctime<='{$et}' order by acctime desc LIMIT {$from_record},{$list_num}";
}
//$query="select * from {$gz_visit_day} WHERE acctime >='{$st}' and acctime<='{$et}' and ip='127.0.0.1' order by acctime desc LIMIT {$from_record},{$list_num}";
...
while($list = $db->fetch_array($result)){
	...
	if($list['dizhi']==''){
		...
		$list['dizhi']=$cop[0];
		$list['network']=$cop[1];
		$dayquery ="update {$gz_visit_day} SET dizhi = '{$cop[0]}',network = '{$cop[1]}' where id = '{$list['id']}'";
		$db->query($dayquery);
	}
	$visit_day[]=$list;
}
