...
if(!$gz_stat && !isset($cs)){header("location:set.php?lang=".$lang.'&anyid='.$anyid);exit;}
/*时间变量*/
$dtime=statime("Y-m-d");	
...
//
$visitday=$db->get_one("SELECT * FROM {$gz_visit_summary} WHERE stattime='{$dtime}'");
if(!$visitday && $gz_stat){
...
//
if($action=='all'){
	$query="select * from {$gz_visit_summary} WHERE stattime >='{$st}' and stattime <='{$et}' order by stattime desc";
	...
	$query="select * from {$gz_visit_summary}";
	$result= $db->query($query);
	while($list = $db->fetch_array($result)){
	...
	$visit_summaryz=$db->get_one("SELECT * FROM {$gz_visit_summary} WHERE stattime='{$ztime}'");
	if(!$visit_summaryz){
		$visit_summaryz['pv']=0;
