...
if($action=='time'||$action=='index-all'||$action=='index-all-line'){
	$query="SELECT * FROM {$gz_visit_summary} WHERE stattime >='{$st}' and stattime<='{$et}'";
...
switch($action){
	case 'time':
		...
		if($ml<0){
			...
			$ztdat=$db->get_one("SELECT * FROM {$gz_visit_summary} WHERE stattime='{$ztime}'");
	...
	case 'accept':
		if($yqnums){
			$query="select * from {$gz_visit_detail} WHERE stattime>='{$st}' and stattime<='{$et}' and type='3' order by pv desc";
			$result= $db->query($query);
			$domain=array();
			while($list = $db->fetch_array($result)){
		...
		for($i=0;$i<count($yqnum);$i++){
			if($yqnum[$i]!=''){
				...
				$t=$t+$p[1];
				$metdata[$i]['value']=$p[1];
				$metdata[$i]['name']=$cny?$gz_langok[$p[0]]['name']:$p[0];
			}
		}
		$h=0;
		foreach($metdata as $key=>$val){
