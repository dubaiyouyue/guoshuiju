...
if($id!=null){
	$querys = "SELECT * FROM $gz_wapmenu where lang='$lang' and id='$id'";
	...
	$querys = "SELECT * FROM $gz_wapmenu where lang='$lang'";
	$menus=$db->query($querys);
	while($list = $db->fetch_array($menus)) {
		$menursr[]=$list;
	}
...
if($action=='editor'){
	...
	if($menu_type==3){
		require_once $depth.'../include/config.php';
		$type=$menu_type;
		$values='[!]'.$gz_maplng.'[!]'.$gz_maplat.'[!]'.$gz_mapzoom.'[!]'.$gz_maptitle.'[!]'.$gz_mapcontents;		
		$querys = "UPDATE $gz_config SET `value`='' where name='gz_maplng'";
		$db->query($querys);
		$querys = "UPDATE $gz_config SET `value`='' where name='gz_maplat'";
		$db->query($querys);
		$querys = "UPDATE $gz_config SET `value`='' where name='gz_mapzoom'";
		$db->query($querys);
		$querys = "UPDATE $gz_config SET `value`='' where name='gz_maptitle'";
		$db->query($querys);
		$querys = "UPDATE $gz_config SET `value`='' where name='gz_mapcontents'";
		$db->query($querys);	
	}
	if($menu_type==4){
		$type=$menu_type;
	...
	if($id!=null){
		$querys = "UPDATE $gz_wapmenu SET `sequence`='$menu_list_order', `name`='$menu_name', `value`='$values',`columnicon`='$gz_icon',`menu_iconrgb`='$menu_iconrgb' ,`menu_wordrgb`='$menu_wordrgb' , `values`='$classs',`type`='$type',`lang`='$lang' where id='$id'";
		$menus=$db->query($querys);
	}else{
		$querys = "INSERT INTO $gz_wapmenu SET `sequence`='$menu_list_order', `name`='$menu_name', `value`='$values',`columnicon`='$gz_icon',`menu_iconrgb`='$menu_iconrgb' ,`menu_wordrgb`='$menu_wordrgb' , `values`='$classs',`type`='$type',`lang`='$lang'";
		$menus=$db->query($querys);
	}
	metsave('../app/wap/custommenu.php?anyid='.$anyid.'&cs='.$cs.'&lang='.$lang,'操作成功',$depth);
}

$querys = "SELECT value FROM $gz_config where name='gz_wapshowtype' and lang='$lang'";
$menus=$db->query($querys);
while($list = $db->fetch_array($menus)) {
	$sql=$list;
}
...
$query = "SELECT * FROM $gz_column where  lang='$lang' $sqls";

$result = $db->query($query);
while($list = $db->fetch_array($result)) {
	$clist[]=$list;
	if(($list['classtype']==1||$list['releclass'])){$clist1now[]=$list;}
	if((($list['classtype']==2&&$list['bigclass']==$class1)||($gz_class[$list['bigclass']]['releclass']&&$list['classtype']==3&&$list['bigclass']==$class1))){$clist2now[]=$list;}
