...
$query="select * from $gz_app where download=1 order by id DESC";
$result = $db->query($query);
while($list= $db->fetch_array($result)){
	$app[$list[no]]=$list;
}

$query="select * from $gz_app where download=0 order by id DESC";
$result = $db->query($query);
while($list= $db->fetch_array($result)){
	$app_dl[$list[no]]=$list;
}
//接口函数
$query="select * from $gz_applist order by id DESC";
$result = $db->query($query);
while($list = $db->fetch_array($result)){
	$list[download] = 1;
	$list[appver] = 1;
