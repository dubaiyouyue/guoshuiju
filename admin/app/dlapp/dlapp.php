		...
		if($info[0]>=$valapps[6]){
			if(metver($metcms_v,$valapps[7],$sysver)>=2){
				if($app[$valapps[1]]['download']==0){
					...
					$valapps['xtype']="<a href=\"http://$gz_host/dl/app.php\" onclick=\"return olupdate('$valapps[1]','0','testc');\"><img src='{$img_url}/dwn.png' /><p>".$typetxt123."{$lang_appinstall}</p></a>";
			...
			$valapps['xtype']="<a href='http://www.resonance.com.cn/' target='_blank'><img src='{$img_url}/dwn.png' /><p>{$mytype[$valapps[6]]}</p></a>";
		}
		$newapplist[]=$valapps;
	}
