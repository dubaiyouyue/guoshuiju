...
function dangerfun($jkdir,$danger,$suffix,$trust){
	global $filenamearray,$physical_function,$db,$gz_column,$url_array;
	@unlink('../../../install/phpinfo.php');
	$physical_function="";
	$adminfile=$url_array[count($url_array)-2];
	deltree(ROOTPATH.'/cache');
	deltree(ROOTPATH."/$adminfile/update");
	$column=$db->get_all("select * from $gz_column where classtype=1 or releclass!=0");
...
function filescan($jkdir,$fileback){
	global $filenamearray,$physical_file,$gz_langok,$db,$gz_column,$url_array;
	$physical_file="";
	$adminfile=$url_array[count($url_array)-2];
	$fileback=parse_ini_file($fileback,true);
	if($adminfile!='admin'){
	...
	foreach($fileback as $key=>$val){
		if(!file_exists('../../../'.$key)){
			...
			if($fileback[$key]['filesize']!=gz_filesize('../../../'.$key)){
				$physical_file .="2|$key|,";
			}
		}
	}
	foreach($filediff as $key=>$val){
		if(!file_exists('../../../'.$key)){
			...
			if($filediff[$key]['filesize']!=gz_filesize('../../../'.$key)){
