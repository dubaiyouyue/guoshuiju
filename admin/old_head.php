...
$_M['url']['adminurl'] = $_M['url']['site'].$gz_adminfile."/index.php?lang={$lang}&";
if($_M[config][gz_agents_type] > 2) $gz_agents_display = "style=\"display:none\"";
...
if($_M['form']['iframeurl']){
	...
	foreach ($sidebarcolumn as $key => $val) {
		...
		//需要清理，下面的代码，有些栏目已经多余
		if ((($val['name'] == 'lang_indexcode') || ($val['name'] == 'lang_indexebook') || ($val['name'] == 'lang_indexbbs') || ($val['name'] == 'lang_indexskinset') ) && $_M['config']['gz_agents_type'] > 1) continue;
		if ((($val['name'] == 'lang_webnanny') || ($val['name'] == 'lang_smsfuc')) && $_M['config']['gz_agents_sms'] == 0) continue;
		if (($val['name'] == 'lang_dlapptips2') && $_M['config']['gz_agents_app'] == 0) continue;
		//
...
echo <<<EOT
-->
<ol class="breadcrumb position hidden-xs" style="display:none;">
...
<div class="btn-group pull-right met-tool met-msecount-tool">
	<button style="display:none;" class="btn btn-default text-center dropdown-toggle msecount" type="button" onclick="location.href = '{$_M[url][site_admin]}index.php?n=system&c=news&a=doindex&lang={$_M[lang]}';">
...
echo <<<EOT
-->
		<li class="met-tool-list" id="yuyanxuanze-{$val['id']}"><a href="{$val['url']}">{$val[name]}</a></li>
<!--
EOT;
}
echo <<<EOT
-->
		...
		<li id="yuyanxuanze-{$val['id']}-q" class="met-tool-list">
...
echo <<<EOT
-->				
		<li class="met-tool-list text-center"><a target="_blank" class="liaojie" href="http://www.resonance.com.cn/">{$_M['word']['sys_authorization2']}</a></li>
		<li style="display:none;" class="met-tool-list text-center">
...
<div style="display:none;" class="btn-group pull-right met-tool supportbox" {$gz_agents_display}>
	<a href="http://www.resonance.com.cn/" class="btn btn-success dropdown-toggle" target="_blank">技术支持<a>
	<!--<button class="btn btn-success dropdown-toggle" type="button" id="dropdownMenu1" data-toggle="dropdown" aria-expanded="true">
		<i class="fa fa-life-ring"></i><span class="hidden-xs">技术支持</span>
		<span class="caret"></span>
		<input name="supporturldata" type="hidden" value="user_key={$_M['config']['gz_secret_key']}&siteurl={$_M['url']['site']}" />
	</button>
	<ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu1">
		...
		<a href="http://www.resonance.com.cn/news/shownews1248.htm" target="_blank">什么是技术支持？</a>
		</li>
