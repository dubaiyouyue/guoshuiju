	...
	if($class3){
		...
		if($class2){
			...
			if($gz_class[$class1]['releclass']){	
				foreach($gz_class3[$class1] as $key=>$val){
					if($val['module']==1)$sqlclass1.=" or id=$val[id] ";
				}
			}else{
				foreach($gz_class2[$class1] as $key=>$val){
					...
					foreach($gz_class3[$val[id]] as $key1=>$val1){
...
if($search == "detail_search"){	
	...
	$total_count = $db->counter($gz_column, "$serch_sql", "*");
}else{
	$total_count = $db->counter($gz_column, "$serch_sql", "*");
}
...
$query = "SELECT * FROM {$gz_column} $serch_sql order by no_order LIMIT $from_record, $list_num";
$result = $db->query($query);
while($list= $db->fetch_array($result)){
	if($list[classtype]==1||$list[releclass])$aboutid=$list[id];
	if($list[classtype]==2&&!$list[releclass])$aboutid=$gz_class[$list[id]][bigclass];
	if($list[classtype]==3){
		if($gz_class[$gz_class[$list[id]][bigclass]][releclass]){
			$aboutid=$gz_class[$list[id]][bigclass];
		}else{
			$aboutid=$gz_class[$gz_class[$list[id]][bigclass]][bigclass];
		}
	}
