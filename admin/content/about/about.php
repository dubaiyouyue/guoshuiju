...
if($action=="modify"){ 
	...
	$db->query($query);
	$html = showhtm($id);
	if($filenameold<>$filename and $metadmin[pagename])deletepage($gz_class[$id][foldername],$id,'about',$updatetimeold,$filenameold);
	...
	$about = $db->get_one("SELECT * FROM $gz_column WHERE id='$id'");
	...
	$ctype = $about[classtype]==2&&count($gz_class3[$id])?1:0;
	$ctype1 = $about[classtype]==2&&count($gz_class3[$id])?1:0;
	if($about[classtype]==3){
		$about2 = $db->get_one("SELECT * FROM $gz_column WHERE id='$about[bigclass]'");
		$class2=$about2[id];
		$class1=$about2[bigclass];
		$ctype1=1;
	}
	$nott=$class1==$class2||$class2==$id?0:1;
	if($gz_class[$class2][releclass]){
		$class1=$class2;
	}
