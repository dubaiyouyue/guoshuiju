...
if($topara){
	...
	gz_setcookie("topara",'',time()-3600);
}
if($gz_content_type == 0){
	$query = "select content_type from {$gz_admin_table} where admin_id='{$metinfo_admin_name}'";
	$gz_content_type1 = $db->get_one($query);
	$gz_content_type = $gz_content_type1['content_type'];
}
$query = "update $gz_admin_table set content_type='$gz_content_type' where admin_id='{$metinfo_admin_name}'";
$db->query($query);
if($gz_content_type!=2){
	if($action=='search'&&$program){	
		...
		foreach ($gz_class2 as $key=>$val){
			foreach ($gz_class2[$key] as $key1=>$val1){
				if($val['module']<9 && !$val['if_in']){
					$contentlistes1[] = $val1;
				}
			}
		...
		foreach ($gz_class3 as $key=>$val){
			foreach ($gz_class3[$key] as $key1=>$val1){
				if($val['module']<9 && !$val['if_in']){
					$contentlistes2[] = $val1;
				}
			}
		...
		foreach($contentlistes2 as $key=>$val){
				switch($val['module']){
					...
					case '2':	
						$column_types2=array();
						foreach($gz_class2 as $key1=>$val1){
							foreach($val1 as $key11=>$val11){								
								if($val11[id]==$val[bigclass]){
									$column_types2=$gz_class1[$key1];
								}
							}							
						}
		...
		if($class1){
			...
			if($gz_class[$class1]['isshow'])$contentlistes[]=$gz_class[$class1];
			
			foreach($gz_class2[$class1] as $key=>$val2){
				$contentlistes[] = $val2;
			}
			foreach($contentlistes as $key=>$val){
				if($val['module']==1){
				$c2 = count($gz_class3[$val['id']]);
				...
				$column_types5=array();
				$column_types5=$db->get_one("select * from $gz_column where id='$val[bigclass]'");
				...
				$sum1='';
				$sums=array();
				$sums=$db->get_one("select count(*) from $gz_cv where lang='$lang' and readok='0'");
				$sum1=$sums['count(*)'];
				if($sum1>99){
					$sum1="99+";
				}
				...
				$sum1='';
				$sums=array();
				$sums=$db->get_one("select count(*) from $gz_message where lang='$lang' and readok='0'");
				$sum1=$sums['count(*)'];
				if($sum1>99){
					$sum1="99+";
				}
				...
				$sum1='';
				$sums=array();
				$sums=$db->get_one("select count(*) from $gz_feedback where class1='$val[id]' and lang='$lang' and readok='0'");
				$sum1=$sums['count(*)'];
				if($sum1>99){
					$sum1="99+";
				}
			...
			$class1=$gz_class[$class2]['bigclass'];
			if($gz_class[$class2]['isshow'])$contentlistes[]=$gz_class[$class2];
			foreach($gz_class3[$class2] as $key=>$val2){
			...
			foreach($contentlistes as $key=>$val){
				...
				switch($val['module']){
					case '1':
						$c2 = count($gz_class2[$val['id']]);
						if($val['releclass'])$c2 = count($gz_class3[$val['id']]);
						$classname = $c2?"class='lt'":'';
						$classname1 = $c2&&$val['isshow']?"class='rt'":'';
						$val['url']='about/about.php?id='.$val[id].'&lang='.$lang.'&anyid='.$anyid;
						$val['set']="<div>";
						...
						$val['set'].='</div>';
						$sum_count=$db->get_all("select * from $gz_column where lang='$lang' and bigclass='$val[id]'");
						foreach ($sum_count as $key=>$val5){
							...
							if($val5[module]==8){
								$sums=array();
								$sums=$db->get_one("select count(*) from $gz_feedback where class1='$val5[id]' and lang='$lang' and readok='0'");
								$sum3=$sums['count(*)'];
							}
							$sum=$sum1+$sum2+$sum3;
							if($sum>99){
					...
					case '5':
						...
						foreach($contentlistes1 as $key=>$val3){
							$column_types5=array();
							$column_types5=$db->get_one("select * from $gz_column where id='$val3[bigclass]'");
	...
	if($module){
		if($class1){
			if($gz_class1[$class1]['isshow']){
				$gz_class1[$class1]['conturl']='about/about.php?id='.$gz_class1[$class1][id].'&lang='.$lang.'&anyid='.$anyid;
				$contentlist[0] = $gz_class1[$class1];
			}
			foreach($gz_class2[$class1] as $key=>$val){
				if($val['module']==$module){
					$val['conturl']='about/about.php?id='.$val[id].'&lang='.$lang.'&anyid='.$anyid;
					if(count($gz_class3[$val['id']]))$val['conturl']="?anyid={$anyid}&lang={$lang}&module=1&class2={$val['id']}";
					$contentlist[] = $val;
				}
			}
		}elseif($class2){
			if($gz_class[$class2]['isshow']){
				$gz_class[$class2]['conturl']='about/about.php?id='.$gz_class[$class2][id].'&lang='.$lang.'&anyid='.$anyid;
				$contentlist[0] = $gz_class[$class2];
			}
			foreach($gz_class3[$class2] as $key=>$val){
				if($val['module']==$module){
					$val['conturl']='about/about.php?id='.$val[id].'&lang='.$lang.'&anyid='.$anyid;
					$contentlist[] = $val;
				}
			...
			switch($module){
				case 1:
					foreach($gz_class1 as $key=>$val){
						if($val['module']==1){
							$val['conturl']='about/about.php?id='.$val[id].'&lang='.$lang.'&anyid='.$anyid;
							if(count($gz_class2[$val['id']]))$val['conturl']="?anyid={$anyid}&lang={$lang}&module=1&class1={$val['id']}";
							$contentlist[] = $val;
						}
					}
				break;
		...
		if(count($gz_classindex[3])!=0 && IN_ADMIN_product){
		...
		if(count($gz_classindex[4])!=0 && IN_ADMIN_download){
		...
		if(count($gz_classindex[5])!=0 && IN_ADMIN_img){
		...
		if(count($gz_classindex[6])!=0 && IN_ADMIN_job){
			$sum=$db->get_one("select count(*) from $gz_cv where lang='$lang' and readok='0'");
			if($sum['count(*)']>99){
				$sum['count(*)']="99+";
			}
			...
			$contentlist[6]['conturl']="job/index.php?class1={$gz_classindex[6][0][id]}&lang={$lang}&anyid={$anyid}";
			$contentlist[6]['cvurl']="job/cv.php?class1={$gz_classindex[6][0][id]}&lang={$lang}&anyid={$anyid}";
		...
		if(count($gz_classindex[7])!=0){
			$sum=$db->get_one("select count(*) from $gz_message where lang='$lang' and readok='0'");
			if($sum['count(*)']>99){
				$sum['count(*)']="99+";
			}
			...
			$contentlist[7]['conturl']="message/index.php?class1={$gz_classindex[7][0][id]}&lang={$lang}&anyid={$anyid}";
		}
		
		if(count($gz_classindex[8])!=0 && IN_ADMIN_feedback){
			$sum=$db->get_one("select count(*) from $gz_feedback where lang='$lang' and readok='0'");
			if($sum['count(*)']>99){
				$sum['count(*)']="99+";
			}
			...
			$contentlist[8]['conturl']="feedback/index.php?class1={$gz_classindex[8][0][id]}&lang={$lang}&anyid={$anyid}";
		}
	}
}
