...
if($action=="editor"){
	...
	if($gz_member_use){
		$lev=$img_list['class3']?$gz_class[$img_list['class3']][access]:($img_list['class2']?$gz_class[$img_list['class2']][access]:$gz_class[$img_list['class1']][access]);
	}
	if(!$img_list)metsave('-1',$lang_dataerror,$depth);
	$query = "select * from $gz_plist where module='5' and listid='$id'";
