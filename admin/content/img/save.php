...
if($filename_okno){
	...
	if($filename!=''){
		...
		$filenameok = $db->get_one("SELECT * FROM $gz_img WHERE ($sql) and filename='$filename'");
		if($filenameok)$metinfo=0;
		if(is_numeric($filename) && $filename!=$id && $gz_pseudo){
			$filenameok1 = $db->get_one("SELECT * FROM {$gz_img} WHERE id='{$filename}' and class1='$class1'");
...
if($filename!='' && $save_type){
		...
		$filenameok = $db->get_one("SELECT * FROM $gz_img WHERE ($sql) {$sql1} and filename='$filename'");
...
$displayimg = $displayimglist;
	...
	if($metadmin[imgother])$query .="
				 ...
				 $query .="top_ok             = '$top_ok'";
			$db->query($query);
	$later_img=$db->get_one("select * from $gz_img where updatetime='$updatetime' and lang='$lang'");
	$id=$later_img[id];
...
if($description){
	$description_type=$db->get_one("select * from $gz_img where id='$id'");
...
if($action=="editor"){
	...
	$htmjs.=indexhtm().'$|$';
	$htmjs.=classhtm($class1,$class2,$class3);
	if($filenameold<>$filename and $metadmin[pagename])deletepage($gz_class[$class1][foldername],$id,'showimg',$updatetimeold,$filenameold);
