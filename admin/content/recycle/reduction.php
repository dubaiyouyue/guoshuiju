...
if($gz_webhtm==2){
   ...
   metsave('../content/recycle/index.php?lang='.$lang.'&anyid='.$anyid,'',$depth);
	...
	if($gz_webhtm!=0&&$gz_htmway==0){
		$query = "SELECT id,class1,class2,class3,filename,recycle FROM $gz_product where lang='$lang' and recycle=3";
		$query .=" UNION SELECT id,class1,class2,class3,filename,recycle FROM $gz_news where lang='$lang' and recycle=2";
		$query .=" UNION SELECT id,class1,class2,class3,filename,recycle FROM $gz_download where lang='$lang' and recycle=4";
		$query .=" UNION SELECT id,class1,class2,class3,filename,recycle FROM $gz_img where lang='$lang' and recycle=5";
		$result = $db->query($query);
		 while($list = $db->fetch_array($result)) {
			$recycle_list[]=$list;
		}
	}
	$query = "update $gz_news set recycle='0',updatetime='".date('Y-m-d H:i:s')."' where lang='$lang' and recycle='2'";
	$db->query($query);
	$query = "update $gz_product set recycle='0',updatetime='".date('Y-m-d H:i:s')."' where lang='$lang' and recycle='3'";
	$db->query($query);
	$query = "update $gz_download set recycle='0',updatetime='".date('Y-m-d H:i:s')."' where lang='$lang' and recycle='4'";
	$db->query($query);
	$query = "update $gz_img set recycle='0',updatetime='".date('Y-m-d H:i:s')."' where lang='$lang' and recycle='5'";
	$db->query($query);
