...
$messagesName1=$db->get_one("select * from $gz_config where name='gz_message_fd_class' and lang='$lang'");
$messagesName2=$db->get_one("select * from $gz_config where name='gz_message_fd_sms' and lang='$lang'");
$messagesName3=$db->get_one("select * from $gz_config where name='gz_message_fd_email' and lang='$lang'");
$query = "SELECT * FROM {$gz_message} {$serch_sql} {$order_sql} LIMIT {$from_record}, {$list_num}";
...
while($list1= $db->fetch_array($result1)){
	for($i=1;$i<4;$i++){
		...
		$query="select * from $gz_mlist where paraid=$messagesName[value] and listid='$list1[id]' and lang='$lang' order by listid desc";
		$result=$db->query($query);
		while($list= $db->fetch_array($result)){
