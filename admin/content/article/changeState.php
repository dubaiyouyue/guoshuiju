...
if($action=='copy'){
	...
	$k=count($allidlist)-1;
	for($i=0;$i<$k; $i++){
		$query="select * from {$gz_news} where id='{$allidlist[$i]}'";
		$copy=$db->get_one($query);
		$copy[content]=str_replace('\'','\'\'',$copy[content]);
		$query = "insert into {$gz_news} set title='$copy[title]',ctitle='$copy[ctitle]',keywords='$copy[keywords]',description='$copy[description]',content='$copy[content]',class1='{$copyclass1}',class2='{$copyclass2}',class3='{$copyclass3}',no_order='$copy[no_order]',wap_ok='$copy[wap_ok]',img_ok='$copy[img_ok]',imgurl='$copy[imgurl]',imgurls='$copy[imgurls]',com_ok='$copy[com_ok]',issue='$copy[issue]',hits='$copy[hits]',updatetime='$copy[updatetime]',addtime='$copy[addtime]',access='{$access}',top_ok='$copy[top_ok]',lang='{$copylang}',recycle='$copy[recycle]',displaytype='$copy[displaytype]',tag='$copy[tag]'";
		$db->query($query);
	}
	...
	$admin_list = $db->get_one("SELECT * FROM $gz_news WHERE id='$id'");
