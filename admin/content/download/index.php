...
$query="select * from $gz_admin_array where array_type='1' and lang='$lang'";
...
if($class1){
	...
	if(count($gz_classindex[$module])==1){
		$class1=$gz_classindex[$module][0]['id'];
		$class1_info=$gz_class[$class1];	
		$sqlclass1=" and class1=$class1  ";
	}else{
		foreach($gz_classindex[$module] as $key=>$val){
...
$order_sql=list_order($gz_class[$classnow][list_order]);
if($search == "detail_search") {			
	...
	$total_count = $db->counter($gz_download, "$serch_sql", "*");
} else {
	$total_count = $db->counter($gz_download, "$serch_sql", "*");
}
...
$query = "SELECT * FROM $gz_download $serch_sql $order_sql LIMIT $from_record, $list_num";
...
include template('content/download/download');
footer();
