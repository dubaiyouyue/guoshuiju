...
if($filename_okno){
	...
	if($filename!=''){
		...
		$filenameok = $db->get_one("SELECT * FROM $gz_download WHERE ($sql) and filename='$filename'");
		if($filenameok)$metinfo=0;
		if(is_numeric($filename) && $filename!=$id && $gz_pseudo){
			$filenameok1 = $db->get_one("SELECT * FROM {$gz_download} WHERE id='{$filename}' and class1='$class1'");
...
if($filename!='' && $save_type){
		...
		$filenameok = $db->get_one("SELECT * FROM $gz_download WHERE ($sql) {$sql1} and filename='$filename'");
...
$query = "INSERT INTO $gz_download SET
                      title              = '$title',
                      ctitle             = '$ctitle',
					  keywords           = '$keywords',
					  ...
					  tag                = '$tag'";
         $db->query($query);
	$later_download=$db->get_one("select * from $gz_download where updatetime='$updatetime' and lang='$lang'");
...
if($description){
	$description_type=$db->get_one("select * from $gz_download where id='$id'");
...
if($metadmin[pagename])$query .= "
	...
	$htmjs.=indexhtm().'$|$';
	$htmjs.=classhtm($class1,$class2,$class3);
	if($filenameold<>$filename and $metadmin[pagename])deletepage($gz_class[$class1][foldername],$id,'showdownload',$updatetimeold,$filenameold);
