...
if($action=="editor"){
	...
	if($gz_member_use){
		$lev=$download_list['class3']?$gz_class[$download_list['class3']][access]:($download_list['class2']?$gz_class[$download_list['class2']][access]:$gz_class[$download_list['class1']][access]);
	}
	if(!$download_list)metsave('-1',$lang_dataerror,$depth);
	$query = "select * from $gz_plist where module='4' and listid='$id'";
	...
	if($download_list[displaytype]==1){
