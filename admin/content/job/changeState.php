...
if($action=='copy'){
	...
	$k=count($allidlist)-1;
	for($i=0;$i<$k; $i++){
		$query="select * from {$gz_job} where id='{$allidlist[$i]}'";
		$copy=$db->get_one($query);
		$copy[content]=str_replace('\'','\'\'',$copy[content]);
		$query = "insert into {$gz_job} set position='$copy[position]',count='$copy[count]',place='$copy[place]',deal='$copy[deal]',addtime='$copy[addtime]',useful_life='$copy[useful_life]',content='$copy[content]',access='$copy[access]',no_order='$copy[no_order]',wap_ok='$copy[wap_ok]',top_ok='$copy[top_ok]',email='$copy[email]',lang='{$copylang}'";
		$db->query($query);
	}
