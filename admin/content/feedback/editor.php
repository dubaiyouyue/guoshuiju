...
if($action=="editor"){
	...
	if(!$class1){
		$feedback_list=$db->get_one("select * from $gz_feedback where id='$id'");
	...
	$query = "update $gz_feedback SET
						  ...
						  where id='$id'";
	$db->query($query);
	$feedback_list=$db->get_one("select * from $gz_feedback where id='$id' and class1 = '$class1'");
	...
	$info_list=$db->get_one("select * from $gz_flist where listid='$id' and paraid='$list[id]' and lang='$lang'");
