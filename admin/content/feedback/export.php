...
# Copyright (C) MetInfo Co.,Ltd (http://www.resonance.com.cn). All rights reserved. 
	...
	$query = "SELECT * FROM $gz_parameter where module=8 and lang='$lang' order by no_order";
	$result = $db->query($query);
	while($list= $db->fetch_array($result)){
		$feedbackpara[$list['id']]=$list;
		$feedback_para[]=$list;
	}	
	$query = "SELECT * FROM $gz_flist where module=8 and lang='$lang'";
	$result = $db->query($query);
	while($list= $db->fetch_array($result)){
		if($feedbackpara[$list['paraid']]['type']==5 and $list[info]<>"")$list[info]=$gz_weburl."upload/file/".$list[info];
		$paravalue[$list[listid]][$list[paraid]]=$list;
	}
	if($gz_fd_export==-1){
		$where=" ";
	}else{
		$where=" and exists(select info from $gz_flist where listid=$gz_feedback.id and paraid=$gz_fd_class and info='$gz_fd_export')";
	}
	$query = "SELECT * FROM $gz_feedback where class1='$class1' and lang='$lang' ".$where;
	$result = $db->query($query);
	while($list= $db->fetch_array($result)){
	...
	$xls->Output($gz_module[8][0][name].".xls");
