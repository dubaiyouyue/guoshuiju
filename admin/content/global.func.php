...
function listjs($module){
	global $db,$gz_column,$lang,$lang_modClass2,$lang_modClass3,$metinfo_admin_pop;
	$query = "SELECT * FROM $gz_column where module='$module' and lang='$lang'";
...
function para_list_with($mod_list){
	global $db,$lang_modnull,$lang_imagename,$gz_class,$class1,$class2,$class3,$gz_list,$gz_parameter,$lang;
	$query = "select * from $gz_parameter where lang='$lang' and module='".$gz_class[$class1]['module']."' and (class1='0' or (class1='$class1' and class2='$class2' and class3=0) or (class1='$class1' and class2='$class2' and class3='$class3') or (class1='$class1' and class2=0 and class3=0)) order by no_order";
	$result = $db->query($query);
	while($list = $db->fetch_array($result)){
		...
		switch($val['type']){
			...
			case 5:
				...
				$val['inputcont']="
					...
					<input name='gz_upsql_{$val[id]}' type='file' id='mod_upload_{$val[id]}' />
...
global $db,$lang_modnull,$lang_imagename,$lang_marks,$gz_class,$class1,$class2,$class3,$gz_list,$gz_parameter,$lang,$gz_plist,$gz_mlist,$lang_Empty,$gz_language,$lang_clickview;
$paras_Empty=$db->get_one("SELECT * FROM $gz_language WHERE name='Empty' and lang='$lang'");
$paras_email=$db->get_one("SELECT * FROM $gz_language WHERE name='js13' and no_order='13' and lang='$lang'");
	if($mun_module==10){
...
foreach($member_list as $key=>$val){
	if($val[type]==4&&$val[wr_ok]==1){
		$query2 = "select * from $gz_list where lang='$lang' and bigid='$val[id]'";
...
if($val[type]==1){
	if($mun_module==7){
	$paras_value=$db->get_one("SELECT * FROM $gz_mlist WHERE listid='$id' and paraid='$val[id]' and module='$mun_module'");
	}else{
	$paras_value=$db->get_one("SELECT * FROM $gz_plist WHERE listid='$id' and paraid='$val[id]' and module='$mun_module'");
	}
	if($val[wr_ok]==1){
...
if($val[type]==2){
	...
	$query = "SELECT * FROM $gz_list WHERE lang='$lang' and bigid='$val[id]'";
	$result = $db->query($query);
