...
if($action=="del"){
	...
	foreach($allidlist as $key=>$val){
		if($gz_webhtm!=0 or $metadmin[pagename]){
			$product_list=$db->get_one("select * from $gz_product where id='$val'");
			if($gz_htmpagename==1)$updatetime=date('Ymd',strtotime($product_list[updatetime]));	
		...
		if(!$gz_recycle){
			$query = "delete from $gz_plist where listid='$val' and module='3'";
			$db->query($query);
		}
		$query = $gz_recycle?"update $gz_product set recycle='3',updatetime='".date('Y-m-d H:i:s')."' where id='$val'":"delete from $gz_product where id='$val'";
		$db->query($query);
		if(!$gz_recycle){if($product_list){delimg($product_list,1,2);}else{delimg($val,3,2);}}
	}
	$htmjs =indexhtm().'$|$';
	...
	$product_list = $db->get_one("SELECT * FROM $gz_product WHERE id='$id'");
	...
	if(!$gz_recycle){
		$query = "delete from $gz_plist where listid='$id' and module='3'";
		$db->query($query);
	}
	$query = $gz_recycle?"update $gz_product set recycle='3',updatetime='".date('Y-m-d H:i:s')."' where id='$id'":"delete from $gz_product where id='$id'";
	$db->query($query);
	if(!$gz_recycle){if($product_list){delimg($product_list,1,2);}else{delimg($id,3,2);}}
	$htmjs =indexhtm().'$|$';
