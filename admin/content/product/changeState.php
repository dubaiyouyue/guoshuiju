...
if($action=='copy'){
	...
	$k=count($allidlist)-1;
	for($i=0;$i<$k; $i++){
		$query = "select * from {$gz_product} where id='{$allidlist[$i]}'";
	...
	foreach($original as $key=>$val){
		...
		$query = "update {$gz_product} SET class1='$moveclass1',class2='$moveclass2',class3='$moveclass3',access='$access',{$filname}lang='$movelang' where id='$val[id]'";
		$db->query($query);
		$query="select * from {$gz_plist} where listid='{$val[id]}'";
		$plist=$db->get_all($query);
		foreach($plist as $key2=>$val2){
	...
	$admin_list = $db->get_one("SELECT * FROM $gz_product WHERE id='$id'");
