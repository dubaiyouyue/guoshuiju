...
if($action=="editor"){
	...
	if($gz_member_use){
		$lev=$product_list['class3']?$gz_class[$product_list['class3']][access]:($product_list['class2']?$gz_class[$product_list['class2']][access]:$gz_class[$product_list['class1']][access]);
	}
	if(!$product_list)metsave('-1',$lang_dataerror,$depth);
	$query = "select * from $gz_plist where module='3' and listid='$id'";
	...
	$lev=$class3?$gz_class[$class1][access]:($class2?$gz_class[$class2][access]:$gz_class[$class1][access]);
}
...
if($gz_class[$class1][releclass]){
	$gz_class22=$gz_class3;
	$gz_class3=array();
}
if(!count($gz_class22[$class1])||!$class1)$class2style="style='display:none'";
if(!count($gz_class3[$class2])||!$class2)$class3style="style='display:none'";
