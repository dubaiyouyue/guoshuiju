...
class upfile {
  ...
  function upload($form, $file = "") {
	global $lang_upfileFail3,$gz_img_rename;
    if (is_array($form)) {
      $filear = $form;
    } else {
      $filear = $_FILES[$form];
  ...
  function setWidthHeight($width, $height, $maxwidth, $maxheight) {
    ...
    {
	     global $gz_img_x,$gz_img_y,$gz_productimg_x,$lang_upfileFail4,$lang_upfileFail5,$lang_upfileFail6,$gz_thumb_kind,$lang_upfileFail7,$lang_upfileFail8,$lang_upfileFail9,$lang_upfileFail10,$lang_upfileFail11;
	if(stristr(PHP_OS,"WIN")){
			$img = @iconv("utf-8","GBK",$img);
		}
		 $thumb_width=$thumb_width?$thumb_width:$gz_img_x;
		 $thumb_height=$thumb_height?$thumb_height:$gz_img_y;
         $gd = $this->gd_version(); 
		...
		$gz_thumb_kind_liubai=0.75;
		$gz_thumb_kind_lashen=0.9;
		if($scale>=$gz_thumb_kind_lashen&&$scale<=(1/$gz_thumb_kind_lashen)){
			$gz_thumb_kind=1;
		}else if($scale<=$gz_thumb_kind_liubai||$scale>=(1/$gz_thumb_kind_liubai)){
			$gz_thumb_kind=2;
		}else{
			$gz_thumb_kind=3;
		}
		*/
		switch($gz_thumb_kind){
			case 1:
				$dst_x=0;
				$dst_y=0;
