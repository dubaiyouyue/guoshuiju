...
$_M['user']['cookie'] = $gz_cookie;
$metinfo_admin_name     = get_gz_cookie('metinfo_admin_name');
$metinfo_admin_pass     = get_gz_cookie('metinfo_admin_pass');
$metinfo_admin_pop      = get_gz_cookie('metinfo_admin_pop');
$metinfo_admin_shortcut = get_gz_cookie('metinfo_admin_shortcut');
$languser               = get_gz_cookie('languser');
$langadminok            = get_gz_cookie('metinfo_admin_lang');
$langusenow=$languser;
if($langadminok<>"" and $langadminok<>'metinfo')$adminlang=explode('-',$langadminok);
require_once ROOTPATH_ADMIN.'include/lang.php';
isset($_REQUEST['GLOBALS']) && exit('Access Error');
unset($_POST['gz_webkeys']);
unset($_GET['gz_webkeys']);
unset($_POST['metinfo_admin_name']);
unset($_GET['metinfo_admin_name']);
unset($_GET['gz_cookie']);
unset($_COOKIE['gz_cookie']);
unset($_POST['gz_cookie']);
...
$metinfoadminfile=ROOTPATH.'templates/'.$gz_skin_user.'/metinfo.inc.php';
...
$gz_htmtypeadmin=($lang==$gz_index_type)?".".$gz_htmtype:"_".$lang.".".$gz_htmtype;
...
//结束
/*管理员权限处理*/
$admin_list = $db->get_one("SELECT * FROM {$gz_admin_table} WHERE admin_id='{$metinfo_admin_name}'");
