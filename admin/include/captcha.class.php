...
$gz_webkeys=trim(file_get_contents(substr(dirname(__FILE__), 0, $safe_len).'/config/config_safe.php'));
...
function CreateCheckCode()
 ...
 global $gz_webkeys;
  ...
  $auth=$this->authcode($this->mCheckCode,'ENCODE', md5($gz_webkeys));
  setcookie("gz_capcha",$auth,time()+3600,'/');
 ...
 */
 ...
 {
	global $gz_webkeys;
  if(!empty($code)&&$this->authcode($_COOKIE['gz_capcha'],'DECODE', md5($gz_webkeys))===$code){
	$this -> CreateCheckCode();
	return true; 
  }else{
	$this -> CreateCheckCode();
