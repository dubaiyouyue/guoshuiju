...
$gz_weburls=explode('/',$gz_weburl);
$url_now=$_SERVER['SERVER_NAME']?$_SERVER['SERVER_NAME']:$_SERVER['HTTP_HOST'];
$domain=$gz_weburl;
if($url_now!=$gz_weburls[2]){
	$domain=str_replace($gz_weburls[2],$url_now,$gz_weburl);
}
$sidebarcolumn=$db->get_all("select * from $gz_admin_column order by type desc,list_order");
foreach($sidebarcolumn as $key=>$val){
	$val['name'] = get_word($val['name']);
	if((($val[name]=='lang_indexcode')||($val[name]=='lang_indexebook')||($val[name]=='lang_indexbbs')||($val[name]=='lang_indexskinset'))&&$gz_agents_type>1)continue;
	if((($val[name]=='lang_webnanny')||($val[name]=='lang_smsfuc'))&&$gz_agents_sms==0)continue;
	if(($val[name]=='lang_dlapptips2')&&$gz_agents_app==0)continue;
			...
			if($metinfo_admin_pop=="metinfo" || $purview=='metinfo'){
				if(strstr($val['url'],"http://")){
					...
					if($val['url']=='/interface/info.php'){
						...
						$val['url']=$domain.$gz_adminfile.$val['url'];
					}
					if(strstr($val['url'],"?")){
...
if(!$shortcut_list){
	...
	change_gz_cookie('metinfo_admin_shortcut',$shortcut_list);
	save_gz_cookie();
	$query="update $gz_admin_table set admin_shortcut='".json_encode($shortcut_list)."' where admin_id='$metinfo_admin_name'";
	$db->query($query);
}
