...
if($hml){
	$hlist = $db->get_one("SELECT * FROM $gz_config WHERE id='{$hml}' and name='metsave_html_list' and lang='{$lang}'");
	$query = "delete from $gz_config where id='{$hml}' and name='metsave_html_list' and lang='$lang'";
	$db->query($query);
	$hml=$hlist['value'];
}
