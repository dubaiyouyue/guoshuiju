...
if($_GET[langset]!="" and $gz_admin_type_ok==1){
$languser = $_GET[langset];
}
$langset=($languser!="")?$languser:$gz_admin_type;
if(!file_get_contents(ROOTPATH.'cache/langadmin_'.$langset.'.php')){
	$js="var user_msg = new Array();\n";
	$query="select * from $gz_language where lang='$langset' and site='1' and array!='0'";
...
$_M[langset]=get_gz_cookie('languser');
$query="select * from $gz_language where site='1' and lang='{$_M[langset]}'";
$languages=$db->get_all($query);
foreach($languages as $key=>$val){
	$_M[word][$val[name]]=$val[value];
}
$query = "SELECT * FROM $gz_config WHERE lang='{$langset}-metinfo'";
$result = $db->query($query);
...
@extract($setagents);
if($gz_agents_type>1){
	$lang_indexthanks=$gz_agents_thanks;
	$lang_metinfo=$gz_agents_name;
	$lang_copyright=$gz_agents_copyright;
	$lang_loginmetinfo=$gz_agents_depict_login;
}
