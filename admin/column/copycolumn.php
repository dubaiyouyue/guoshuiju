...
for($i=0;$i<$adnum;$i++){
	$valid=$allidlist[$i];
	if($gz_class[$valid][classtype]==3){
		$first_column=$gz_class[$gz_class[$valid][bigclass]][bigclass];
	}
	if($gz_class[$valid][classtype]==2){
		$first_column=$gz_class[$valid][bigclass];
	}
	if($gz_class[$valid][classtype]==1){
		$first_column=$valid;
	}
...
//复制栏目
for($i=0;$i<$adnum;$i++){
	$query="select * from $gz_column where id='$allidlist[$i]'";
	$column=$db->get_one($query);
	$query="select * from $gz_column where foldername='$column[foldername]' and lang='$copylang' and (classtype='1' or releclass!='0')";
	$foldername_now=$db->get_one($query);
	if($foldername_now&&$cpoy_insert_id[$foldername_now[id]]!=1){
		if($column[module]==0){
			$query="select * from $gz_column where out_url='$column[out_url]' and module='$column[module]' and lang=''";
	...
	$query="INSERT INTO $gz_column set 
			name                     ='$column[name]',
	...
	if($column[classtype]==1||$column[releclass]!=0){
		...
		if($column[module]==6||$column[module]==7||$column[module]==8){
			$query="select * from $gz_config where columnid='$column[id]' and flashid='0'";
			$config_para=$db->get_all($query);
			foreach($config_para as $key=>$val){
				$para_name=$val[name];
				$query="update $gz_config set value='$val[value]' where columnid='{$insert_id}' and flashid='0' and name='{$para_name}'";
				$db->query($query);
			}
		}
		if($column[module]==3||$column[module]==4||$column[module]==5||$column[module]==6||$column[module]==8){
			$query = "select * from $gz_parameter where (class1='0' or class1='$column[id]') and module='$column[module]' and lang='$lang_now'";
			...
			foreach($para_list as $key=>$val){
				...
				$query="insert into $gz_parameter set name='$val[name]',description='$val[description]',no_order='$val[no_order]',type='$val[type]',access='$val[access]',wr_ok='$val[wr_ok]',class1='$val[class1]',module='$val[module]',lang='$copylang'";
				$db->query($query);
				$copy_para_list[$val[id]]=$db->insert_id();
				if($val[type]==2||$val[type]==4||$val[type]==6){
					$query="select * from $gz_list where bigid='$val[id]'";
					$slist_list=$db->get_all($query);
					$paraid=$db->insert_id();
					foreach($slist_list as $key1=>$val1){
						$val1[info]=str_replace('\'','\'\'',$val1[info]);
						$query="insert into $gz_list set bigid='{$copy_para_list[$val[id]]}',info='$val1[info]',no_order='$val1[no_order]',lang='$copylang'";
						$db->query($query);
					}
				}
			}
...
foreach($cpoy_bigclass_id as $key=>$val){
	$query="update $gz_column set bigclass='$cpoy_list_id[$val]' where id='$key'";
	$db->query($query);
}
//复制内容
if($copycontent){
	foreach($content_class1 as $key=>$val){
		...
		switch($val[module]){
			case 2:
				$query = "SELECT * FROM {$gz_news} where class1='$val[id]'";
			...
			break;
			case 4:
				$query = "SELECT * FROM {$gz_download} where class1='$val[id]'";
				$result = $db->query($query);
				while($list= $db->fetch_array($result)){
					$list[content]=str_replace('\'','\'\'',$list[content]);
					$list[title]=str_replace('\'','\'\'',$list[title]);
			...
			break;
			case 5:
				$query = "SELECT * FROM {$gz_img} where class1='$val[id]'";
				$result = $db->query($query);
				while($list= $db->fetch_array($result)){
					$list[content]=str_replace('\'','\'\'',$list[content]);
					$list[content1]=str_replace('\'','\'\'',$list[content1]);
					...
					$list[keywords]=str_replace('\'','\'\'',$list[keywords]);
					$list[description]=str_replace('\'','\'\'',$list[description]);
					$query = "insert into {$gz_img} set title='$list[title]',ctitle='$list[ctitle]',keywords='$list[keywords]',description='$list[description]',content='$list[content]',class1='{$cpoy_list_id[$list[class1]]}',class2='{$cpoy_list_id[$list[class2]]}',class3='{$cpoy_list_id[$list[class3]]}',no_order='$list[no_order]',wap_ok='$list[wap_ok]',new_ok='$list[new_ok]',imgurl='$list[imgurl]',imgurls='$list[imgurls]',displayimg='$list[displayimg]',com_ok='$list[com_ok]',hits='$list[hits]',updatetime='$list[updatetime]',addtime='$list[addtime]',issue='$list[issue]',access='$list[access]',top_ok='$list[top_ok]',lang='{$copylang}',content1='$list[content1]',content2='$list[content2]',content3='$list[content3]',content4='$list[content4]',contentinfo='$list[contentinfo]',contentinfo1='$list[contentinfo1]',contentinfo2='$list[contentinfo2]',contentinfo3='$list[contentinfo3]',contentinfo4='$list[contentinfo4]',recycle='$list[recycle]'";
					$db->query($query);
					$insert_contents_id=$db->insert_id();
					$query="select * from {$gz_plist} where listid='{$list[id]}'";
					$plist=$db->get_all($query);
			...
			break;
			case 6:
				$query = "SELECT * FROM {$gz_job} where lang='$lang_now'";
				$result = $db->query($query);
				while($list= $db->fetch_array($result)){	
					...
					$list[content]=str_replace('\'','\'\'',$list[content]);
					$query = "INSERT INTO {$gz_job} SET position='$list[position]',count='$list[count]',place='$list[place]',deal='$list[deal]',addtime='$list[addtime]',useful_life='$list[useful_life]',content='$list[content]',access='$list[access]', top_ok='$list[top_ok]',wap_ok='$list[wap_ok]',filename='',email='$list[email]',no_order='$list[no_order]',lang='$copylang'";
			        $db->query($query);
				}
			break;
			case 7:
				$query = "SELECT * FROM {$gz_message} where lang='$lang_now'";
				$result = $db->query($query);
				while($list= $db->fetch_array($result)){
					$list[customerid]=str_replace('\'','\'\'',$list[customerid]);
					$query = "INSERT INTO {$gz_message} SET ip='$list[ip]',addtime='$list[addtime]',readok='$list[readok]',useinfo='$list[useinfo]',lang='$copylang',access='$list[access]',customerid='$list[customerid]'";
			        $db->query($query);
				}
				$query = "SELECT * FROM {$gz_parameter} where lang='$lang_now' and module=7 ORDER BY id";
				$result = $db->query($query);
				while($parameter= $db->fetch_array($result)){
					$query = "INSERT INTO {$gz_parameter} SET name='$parameter[name]',description='$parameter[description]',no_order='$parameter[no_order]',type='$parameter[type]',access='$parameter[access]',wr_ok='$parameter[wr_ok]',class1='$parameter[class1]',class2='$parameter[class2]',class3='$parameter[class3]',module='$parameter[module]',lang='$copylang',wr_oks='$parameter[wr_oks]'";
					$db->query($query);
				}
				$query = "SELECT id FROM $gz_message where lang='{$copylang}' ORDER BY id";
				$result = $db->query($query);
				while($list1= $db->fetch_array($result)){
					$mlist_id[]=$list1[id];
				}
				$query = "SELECT id FROM $gz_parameter where lang='{$copylang}' and module=7 ORDER BY id";
				$result = $db->query($query);
				while($list2= $db->fetch_array($result)){
					$paraid_id[]=$list2[id];
				}
				$query = "SELECT * FROM {$gz_mlist} where lang='$lang_now' and module=7 order by listid and id";
				$result = $db->query($query);
				$count= 0 ;
				$i = 0;
				$j = 0;
				while($mlist= $db->fetch_array($result)){
					$count+=1;
					$query = "INSERT INTO {$gz_mlist} SET listid='$mlist_id[$i]',paraid='$paraid_id[$j]',info='$mlist[info]',lang='$copylang',imgname='$mlist[imgname]',module=7";
					$db->query($query);
			...
			break;
			case 8:
				$query = "SELECT * FROM {$gz_feedback} where class1='$val[id]'";
				$result = $db->query($query);
				while($list= $db->fetch_array($result)){
					...
					$list[customerid]=str_replace('\'','\'\'',$list[customerid]);
					$query = "INSERT INTO {$gz_feedback} SET class1='$insert_id',fdtitle='$list[fdtitle]',fromurl='$list[fromurl]',ip='$list[ip]',addtime='$list[addtime]',readok='$list[readok]',useinfo='$list[useinfo]',customerid='$list[customerid]',lang='$copylang'";
			        $db->query($query);
				}
			break;
			case 9:
				$query = "SELECT * FROM {$gz_link} where lang='$lang_now'";
				$result = $db->query($query);
				while($list= $db->fetch_array($result)){
					...
					$query = "INSERT INTO {$gz_link} SET webname='$list[webname]',weburl='$list[weburl]',weblogo='$list[weblogo]',link_type='$list[link_type]',info='$list[info]',contact='$list[contact]',orderno='$list[orderno]',com_ok='$list[com_ok]',show_ok='$list[show_ok]',addtime='$list[addtime]',lang='$copylang',ip='$list[ip]'";
			        $db->query($query);
				}
			break;
		}
