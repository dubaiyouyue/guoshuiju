...
if($action=="del"){
	...
	foreach($allidlist as $key=>$val){
		$admin_list = $db->get_one("SELECT * FROM $gz_column WHERE id='$val'");
		if($admin_list){
			$query1 = "select * from $gz_column where bigclass='$admin_list[id]'";
			$result1 = $db->query($query1);
			while($list1= $db->fetch_array($result1)){
				if($list1['releclass']||$list1['classtype']==3){
					delcolumn($list1);
				}else{
					$query2 = "select * from $gz_column where bigclass='$list1[id]'";
		...
		if($admin_list[module]>1000){
			...
			$query = "delete from $gz_ifmember_left where columnid='$val'";
			$db->query($query);
		}
	}
	...
	for($i=0;$i<$adnum;$i++){
		...
		if($module>1000){
		$fixed=$db->get_one("select * from $gz_ifcolumn where no='$module'");
		if($fixed[fixed_name]){
			$foldername=$fixed[fixed_name];
		}
		$out_url = $gz_weburl.$foldername;
		$if_in=1;
		}
		...
		if($sql!=''){
			$skin_m=$db->get_one("SELECT * FROM $gz_column WHERE $sql");
		...
		$releclassok=$db->get_one("SELECT * FROM $gz_column WHERE id='$bigclass'");
		if($classtype==2){
		...
		if($if_in==0){
			...
			if($tpif){
				...
				if($filename!=''){
					$filenameok = $db->get_one("SELECT * FROM $gz_column WHERE filename='$filename'");
				...
				if(!preg_match('/^[a-z0-9_-]+$/i',$foldername)){
					...
					if($module>5 && $module!=8){
						$modulewy = $db->get_one("SELECT * FROM $gz_column WHERE module='$module' and lang='$lang'");
						if($modulewy['id'])$metinfo.=$ertxt.'module_'.$allidlist[$i].'|'.$lang_modmodulewyok.'$';
					}
					if($bigclass==0 && (($module>0 && $module<6) || $module==8))$releok=1;
					if($releok){
						$folder_m=$db->get_one("SELECT * FROM $gz_column WHERE foldername='$foldername' and lang='$lang'");
						if($folder_m){
							if($module<13 && file_exists($filedir))$metinfo.=$ertxt.'foldername_'.$allidlist[$i].'|'.$lang_columnerr4.'$';
						}elseif(file_exists($filedir)){
							$folder_m=1;
						}
						$folder_ms=$db->get_one("SELECT * FROM $gz_column WHERE foldername='$foldername' and lang!='$lang'");
						if($folder_ms){
	...
	foreach($column as $key=>$val){
		...
		$column_id=$db->get_one("select * from $gz_column where name='$val[name]' and lang='$lang'");
		if($val[module]>1000){
			establish_appmodule($val[foldername],$val[module]);
			$module_Information=$db->get_one("select * from $gz_ifcolumn where no='$val[module]'");
			if($module_Information[memberleft]==1){
				if($uptp=="insert into"){
					$query="insert into $gz_ifmember_left set no='$val[module]',columnid='$column_id[id]',title='$column_id[name]',foldername='$val[foldername]',filename='index.php'";
					$db->query($query);
				}
			}
		}
	...
	if($metinfo_admin_pop1!=''){
		if($metinfo_admin_pop!="metinfo"){
			...
			$query = "update $gz_admin_table SET admin_type = '$metinfo_admin_pop1' where id='$admin_list[id]'";
			$db->query($query);
		}
		$admin_list = $db->get_all("SELECT * FROM $gz_admin_table where usertype = 3 && admin_type!='metinfo' &&  admin_type like '%9999%' and id!='$admin_list[id]'");
		foreach($admin_list as $key=>$val){
			$val['admin_type1']=$val['admin_type'].$metinfo_admin_pop1;
			$val['admin_type1']=metdetrim($val['admin_type1']);
			$query = "update $gz_admin_table SET admin_type = '$val[admin_type1]' where id='$val[id]'";
			$db->query($query);
		}
	}
	file_unlink("../../cache/column_$lang.inc.php");
...
metsave('../column/index.php?anyid='.$anyid.'&lang='.$lang,'','','',$gent);
	...
	$admin_list = $db->get_one("SELECT * FROM {$gz_column} WHERE id='$id'");
	...
	if($admin_list[module]>1000){
		deldir("../../".$admin_list[foldername]);
		$query = "delete from $gz_ifmember_left where columnid='$id'";
		$db->query($query);
	}
	delcolumn($admin_list);
	file_unlink("../../cache/column_$lang.inc.php");
