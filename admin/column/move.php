...
$movenow=$db->get_one("select * from $gz_column where id='$id'");
if($action=='move'){
$classto=$class2?$class2:$class1;
$moveto=$db->get_one("select * from $gz_column where id='$classto'");
...
/*非关联*/
	...
	if($classto==0){/*移动为顶级栏目*/
		...
		$query="update $gz_column set classtype=1,bigclass=0,foldername='$foldername',releclass=0 where id='$id'";
		$db->query($query);
		if($metinfo_admin_pop!="metinfo"){
			$metinfo_admin_pop.=$id.'-';
			$query = "update $gz_admin_table SET admin_type = '$metinfo_admin_pop' where id='$admin_list[id]'";
			$db->query($query);
		}
		$query="update $table set class1=$movenow[id],class2=0,class3=0 where $classold1";
		$db->query($query);
		$query="select * from $gz_column where bigclass='$movenow[id]'";
		...
		$query="update $gz_column set classtype=2,foldername='$foldername',releclass=0 where bigclass='$id'";
		$db->query($query);
		...
		$moveto['classtype']+=1;
		$query="update $gz_column set classtype=$moveto[classtype],bigclass=$moveto[id],foldername='$moveto[foldername]' where id='$id'";
		$db->query($query);
		...
		$query="update $gz_column set classtype=$moveto[classtype],foldername='$moveto[foldername]' where bigclass='$id'";
		$db->query($query);		
		/*delete foldername*/
		$admin_lists = $db->get_one("SELECT * FROM $gz_column WHERE foldername='$movenow[foldername]'");
	...
	if($classto==0){
		$query="update $gz_column set classtype=1,bigclass=0,releclass=0 where id='$id'";
		$db->query($query);
		$query="update $gz_column set classtype=2 where bigclass=$movenow[id]";
		$db->query($query);
	}
	else{
		if($moveto['classtype']==1){
			$query="update $gz_column set classtype=2,bigclass=$moveto[id],releclass=$moveto[id] where id='$id'";
			$db->query($query);
			$query="update $gz_column set classtype=3 where bigclass=$movenow[id]";
			$db->query($query);
		}
		else{
			$query="update $gz_column set classtype=3,bigclass=$moveto[id],releclass=$moveto[id] where id='$id'";
			$db->query($query);
		}
	}
	file_unlink("../../cache/column_$lang.inc.php");
	...
	$query = "SELECT * FROM $gz_column where lang='$lang'";
