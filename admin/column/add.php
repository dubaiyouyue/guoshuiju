...
if($pk){
if((count($gz_module[$j])==0 or ($j<=5 || $j==8)) && $j != 100 && $j != 101){
		if($langmod1) $newlist.= "<option value='$j'>{$langmod1}</option>";
		//栏目管理隐藏显示
}}}
		$newlist.= "<option value='999'>{$lang_modout}</option>";
if($type!=3){	
		$structure=$db->get_all("select * from $gz_ifcolumn");		
		foreach($structure as $key=>$val){
			$structure1=$db->get_one("select * from $gz_column where module='$val[no]' and lang='$lang'");
