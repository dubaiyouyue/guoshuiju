...
if($column_list[classtype]!=1&&$column_list[module]==$gz_class[$column_list[bigclass]][module])$releclassok="disabled='disabled'";
if(count($gz_class2[$column_list[id]]))$releclassok="disabled='disabled'";
else{$if_out_p="none";}
...
$class2_list = $db->get_one("SELECT * FROM $gz_column WHERE id='$column_list[bigclass]'");
...
$displays= $db->get_one("SELECT * FROM $gz_column WHERE id='$id'");
...
if($displays[is_inindex]==0){
	$is_inindex[0]='checked';
	$is_inindex[1]='';
}else{
	$is_inindex[0]='';
	$is_inindex[1]='checked';
}
