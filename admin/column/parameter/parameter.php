...
if($action=="editor"){
	...
	$query="delete from $gz_parameter where id='$id'";
	$db->query($query);
	if($type==2 or $type==4 or $type==6){
		$query="delete from $gz_list where bigid='$id'";
		$db->query($query);
	}
	/*delete images*/
	if($type==5){
		$query="select * from $gz_plist where paraid='$id'";
		$result= $db->query($query);
		while($list = $db->fetch_array($result)){
			file_unlink("../../".$list[info]);
		}
	}
	$query="delete from $gz_plist where paraid='$id'";
	$db->query($query);
	metsave('../column/parameter/parameter.php?anyid='.$anyid.'&module='.$module.'&lang='.$lang.'&class1='.$class1.'&cs='.$cs,'',$depth);
}elseif($action=="delete"){
	$allidlist=explode(',',$allid);
	foreach($allidlist as $key=>$val){
		$para_list = $db->get_one("SELECT * FROM $gz_parameter WHERE id='$val'");
		$type = $para_list['type'];
		$query="delete from $gz_parameter where id='$val'";
		$db->query($query);
		if($type==2 or $type==4 or $type==6){
			$query="delete from $gz_list where bigid='$val'";
			$db->query($query);
		}
		/*delete images*/
		if($type==5){
			$query="select * from $gz_plist where paraid='$val'";
			$result= $db->query($query);
			while($list = $db->fetch_array($result)){
				file_unlink("../../".$list[info]);
			}
		}
		$query="delete from $gz_plist where paraid='$val'";
		$db->query($query);
		$type='';
	}
	metsave('../column/parameter/parameter.php?anyid='.$anyid.'&module='.$module.'&lang='.$lang.'&class1='.$class1.'&cs='.$cs,'',$depth);
	...
	$newslit.= "<option value='0' selected='selected'>$lang_allcategory</option>\n";
		foreach($gz_classindex[$module] as $key=>$val1){
			...
			foreach($gz_class2[$val1['id']] as $key=>$val2){
    ...
    $query="select * from $gz_parameter where module='$module' and lang='$lang'  order by no_order";
	if($class1)$query="select * from $gz_parameter where module='$module' and (class1='$class1' or class1='0') and lang='$lang'  order by no_order";
	if($class2)$query="select * from $gz_parameter where module='$module' and (class1='0' or (class1='$class1' and class2='0') or (class1='$class1' and class2='$class2')) and lang='$lang'  order by no_order";
	if($class3)$query="select * from $gz_parameter where module='$module' and (class1='0' or (class1='$class1' and class2='$class2' and class3='0') or (class1='$class1' and class2='$class2' and class3='$class3')) and lang='$lang'  order by no_order";
	$result= $db->query($query);
