...
if($action=="add"){
	$list_if=$db->get_one("SELECT * FROM $gz_list WHERE info='$info' and bigid='$bigid' ");
	if($list_if)metsave('-1',$lang_dataerror,$depth);
	$list_ok=$db->get_one("SELECT * FROM $gz_list WHERE bigid='$bigid' and no_order=99999 ");
	if($listproduct=="metinfo" && $list_ok){
		$query=(trim($info)==0)?"delete from $gz_list where bigid='$bigid' and no_order=99999":"update $gz_list set info='$info' where bigid='$bigid' and no_order=99999";
	}else{
	   $query="insert into $gz_list set
			   info     ='$info',
			   no_order ='$no_order',
			   lang     ='$lang',
			   bigid    ='$bigid'";
	...
	for($i=0;$i<$adnum;$i++){
		...
		if($sql!='')$skin_m=$db->get_one("SELECT * FROM $gz_list WHERE $sql");
		if($tpif){
		...
		if($tpif){
			$query="update $gz_plist set info='$info' where paraid='$skin_m[bigid]' and info='$skin_m[info]' and lang='$lang'";
			$db->query($query);
		}
		$uptp = $tpif?"update":"insert into";
		$upbp = $tpif?"where id='$allidlist[$i]'":",lang='$lang'";
		$query="$uptp $gz_list set
				info       ='$info',
				no_order   ='$no_order',
				bigid      ='$bigid'
				$upbp";
	...
	if($action_type=="del"){
		...
		foreach($allidlist as $key=>$val){
			$query = "delete from {$gz_list} where id='{$val}'";
			$db->query($query);
		}
	}else{
		$query="delete from $gz_list where id='$id'";
		$db->query($query);
	}
	metsave($backurl,'',$depth);
}else{
	$listinfo='';
	$bigid_info=$db->get_one("select * from $gz_parameter where id='$bigid'");
	if($bigid_info['module']==8){
		$listinfo=$db->get_one("select * from $gz_list where bigid='$bigid' and no_order=99999");
		$listinfoid=intval(trim($listinfo['info']));
		
	}
	if($listinfo){
		$listmarknow='metinfo';
		$classtype=($listinfo[info]=='metinfoall')?$listinfoid:($gz_class[$listinfoid][releclass]?'class1':'class'.$gz_class[$listinfoid][classtype]);
		$query = "SELECT title FROM $gz_product where $classtype=$listinfoid and lang='$lang'  and recycle='0' order BY updatetime desc";
		...
		$query = "SELECT * FROM $gz_list  where bigid='$bigid' order BY no_order";
		$result = $db->query($query);
		while($list = $db->fetch_array($result)) {
			$fd_list[]=$list;
		}
