...
if(!$gz_addlinkopen)okinfo('javascript:history.back();',$lang_ApplyLinkNO);
    $link_list=$db->get_one("select * from $gz_column where module='9' and lang='$lang'");
...
require_once ROOTPATH.'include/head.php';
	...
	$addlink_url=$gz_pseudo?'addlink-'.$lang.'.html':($gz_webhtm?"addlink".$gz_htmtype:"addlink.php?lang=".$lang);
    if($action=="add"){
        ...
        $ip=$m_user_ip;
        $addtime=$m_now_date;
        $ipok=$db->get_one("select * from $gz_link where ip='$ip' order by addtime desc");
        if($ipok)
...
sendsms($gz_nurse_link_tel,$message,4);
}
}
/**/
        $query = "INSERT INTO $gz_link SET
                      webname              = '$webname',
					  info                 = '$info',
					  link_type            = '$link_type',
					  weburl               = '$weburl',
		...
		if($link_list['ctitle']!='')$gz_title=$link_list['ctitle'];
		...
		$fdjs=$fdjs."var webnamelength = document.myform.webname.value.replace(/(^\s*)|(\s*$)/g, '')";
		$fdjs=$fdjs."var weburllength = document.myform.weburl.value.replace(/(^\s*)|(\s*$)/g, '')";
		$fdjs=$fdjs."if (webnamelength == 0) {\n";
        $fdjs=$fdjs."alert('{$lang_LinkInfo2}');";
        $fdjs=$fdjs."document.myform.webname.focus();";
        $fdjs=$fdjs."return false;}";
        $fdjs=$fdjs."if (weburllength == 0 || weburllength == 'http://') {";
...
$methtml_addlink.="<td width='70%' bgcolor='#FFFFFF' class='addlink_td2'>".$gz_linkname."&nbsp;</td>\n";
...
$methtml_addlink.="<td bgcolor='#FFFFFF' class='addlink_td2'>".$gz_weburl."</td>\n";
$methtml_addlink.="<td bgcolor='#FFFFFF' style='color:#990000'></td>\n";
$methtml_addlink.="</tr>\n";
$methtml_addlink.="<tr class='addlink_tr'>\n";
$methtml_addlink.="<td align='right' bgcolor='#FFFFFF' class='addlink_td1'><b>".$lang_OurWebLOGO."</b>&nbsp;</td>\n";
$methtml_addlink.="<td bgcolor='#FFFFFF' class='addlink_td2'><img src='".$gz_logo."' height='33' /></td>\n";
$methtml_addlink.="<td bgcolor='#FFFFFF' style='color:#990000'></td>\n";
$methtml_addlink.="</tr>\n";
$methtml_addlink.="<tr class='addlink_tr'>\n";
$methtml_addlink.="<td align='right' bgcolor='#FFFFFF' class='addlink_td1'><b>".$lang_OurWebKeywords."</b>&nbsp;</td>\n";
$methtml_addlink.="<td bgcolor='#FFFFFF' class='addlink_td2'>".$gz_title_keywords."</td>\n";
$methtml_addlink.="<td bgcolor='#FFFFFF' style='color:#990000'></td>\n";
$methtml_addlink.="</tr>\n";
