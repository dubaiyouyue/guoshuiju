...
if($gz_member_use)$query = "SELECT * FROM $gz_parameter where lang='$lang' and  module=6  and access<='$metinfo_member_type' order by no_order";
...
$fdjs=$fdjs."</script>";
	...
	$query = "SELECT id,position FROM $gz_job $serch_sql and  access <= '$metinfo_member_type' and ((TO_DAYS(NOW())-TO_DAYS(`addtime`)< useful_life) OR useful_life=0)";
...
$class_info[name]=$class2_info[name]."--".$class1_info[name];
	 ...
	 $gz_title=$gz_title?$lang_cvtitle.'-'.$gz_title:$lang_cvtitle;
if(!$guanlian){
     if(count($nav_list2)){
