...
$ip=$m_user_ip;
$addtime=$m_now_date;
$ipok=$db->get_one("select * from $gz_cv where ip='$ip' order by addtime desc");
if($ipok)
...
if($timeok<=$gz_cv_time&&$timeok2<=$gz_cv_time){
$fd_time="{$lang_Feedback1}".$gz_cv_time."{$lang_Feedback2}";
...
if($gz_cv_type==1 or $gz_cv_type==2){
$query = "INSERT INTO $gz_cv SET addtime = '$m_now_date', customerid = '$customerid',jobid='$jobid',lang='$lang',ip='$ip' ";
$db->query($query);
$news_id=$db->insert_id();
$news_type = "job";
$new_time = time();
$news_info = $db->get_one("select * from $gz_job where id='{$jobid}'");
...
$query = "INSERT INTO $gz_infoprompt SET
				  ...
				  lang              = '$lang'";					  
$db->query($query);
$later_cv=$db->get_one("select * from $gz_cv where lang='$lang' order by addtime desc");
$id=$later_cv[id];
foreach($cv_para as $key=>$val){
    if($val[type]!=4){
	  $para=$$val[para];
...
if($gz_cv_type==0 or $gz_cv_type==2){
    $job_list = $db->get_one("SELECT * FROM $gz_job WHERE id='$jobid'");
    $from=$gz_fd_usename;
    $fromname=$gz_fd_fromname;
    $to=$gz_cv_emtype?($job_list[email]!=''?$job_list[email]:$gz_cv_to):$gz_cv_to;
    $usename=$gz_fd_usename;
    $usepassword=$gz_fd_password;
    $smtp=$gz_fd_smtp;
    $title=$lang_cv2.$job_list[position].'('.$gz_weburl.')';
...
$bt=$imgurl[1]!=''?'<td class="pc" rowspan="5">'.'<img src="'.$gz_weburl.$imgurl[1].'" width="140" height="160" /></td>':'';
foreach($cv_para as $key=>$val){
$j++;
if($j>1)$bt = '';
    if($val[type]!=4){
...
if($gz_cv_image!=$val[id]){
$para=explode('../',$para);
$para=$para[1]<>""?"<a href=".$gz_weburl.$para[1]." trage='_blank' style='color:#f00;' >".$lang_Download."</a>":$lang_Emptyno;
$body=$body.'<tr><td class="l">'.$val[name].'</td><td class="r">'.$para.'</td>'.$bt.'</tr>'."\n";
}
}
}
$body.='<tr><td class="footer" colspan="3">Powered by <a target="_blank" href="http://www.resonance.com.cn">MetInfo '.$metcms_v.'</a> &copy;2012-2011 &nbsp;<a target="_blank" href="http://www.resonance.com.cn">MetInfo Inc.</a></td></tr>';
$body.='</table>';
if($gz_cv_back==1){
jmailsend($from,$fromname,$cvto,$gz_cv_title,$gz_cv_content,$usename,$usepassword,$smtp);
}
...
sendsms($gz_nurse_job_tel,$message,4);
}
}
/**/
