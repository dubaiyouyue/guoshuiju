...
$serch_sql = '';
$order_sql = '';
$search_column=$db->get_one("select * from $gz_column where module='11' and lang='$lang'");
...
case 0:   
	$pagename="news";
    $query = "SELECT $searchitem FROM $gz_news $serch_sql order by updatetime desc";
	...
	$pagename="product";
    $query = "SELECT $searchitem FROM $gz_product $serch_sql order by updatetime desc";
    $result = $db->query($query);
	while($list= $db->fetch_array($result)){
	$filename=$navurl.$class_list[$list[class1]][foldername];
	...
	$pagename="download";
    $query = "SELECT $searchitem1 FROM $gz_download $serch_sql order by updatetime desc";
    $result = $db->query($query);
	while($list= $db->fetch_array($result)){
	$filename=$navurl.$class_list[$list[class1]][foldername];
	...
	$pagename="img";
    $query = "SELECT $searchitem FROM $gz_img $serch_sql order by updatetime desc";
    $result = $db->query($query);
	while($list= $db->fetch_array($result)){
	$filename=$navurl.$class_list[$list[class1]][foldername];
	$filenamenow="showimg";
...
break;
	...
	$query1 = "SELECT * FROM $gz_column $serch_sql1 order by id";
    $result = $db->query($query1);
	while($list= $db->fetch_array($result)){
		//if($list[filename]=='')$list[filename]=$list[foldername].$list[id];
		$url1="../".$list[foldername]."/show.php?lang=".$lang."&id=".$list[id];
		$url2=$list[filename]?"../".$list[foldername]."/".$list[filename].$gz_htmtype:"../".$list[foldername]."/".$list[foldername].$list[id].$gz_htmtype;	
		$url3=$list[filename]?"../".$list[foldername]."/".$list[filename].'-'.$lang.$gz_htmtype:"../".$list[foldername]."/".$list[id].'-'.$lang.$gz_htmtype;	
		$list[url]=$gz_pseudo?$url3:($gz_webhtm?$url2:$url1);
 ...
 switch($searchtype){
   ...
   break;
	...
	if($module_name<2||$module_name>9)okinfo('javascript:history.back();',$lang_js1);
  	$table_name="gz_".$modulename[$module_name][0];
	...
	while($list= $db->fetch_array($result)){
    $filename=$navurl.$class_list[$list[class1]][foldername];
	$filenamenow=($gz_htmpagename==2)?$class_list[$list[class1]][foldername]:($gz_htmpagename?date('Ymd',strtotime($list[addtime])):$modulename[$module_name][1]);
    require 'searchlist.php';
	}
...
global $search_list,$gz_img_x,$gz_img_y,$lang_Detail;
   $methtml_searchlist.="<ul>\n";
  foreach($search_list as $key=>$val){
  if($img)$methtml_searchlist.="<span class='search_img'><a href='".$val[url]."' target='_blank'><img src='".$val[imgurls]."' width=".$gz_img_x." height=".$gz_img_y." /></span>";
