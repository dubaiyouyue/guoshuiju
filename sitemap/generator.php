...
if($htmsitemap==$gz_member_force && ($gz_sitemap_html||$gz_sitemap_xml||$gz_sitemap_txt)){
	if($gz_sitemap_lang){
		$lang_now=$lang;
		$gz_weburl_now=$gz_weburl;
		$gz_webname_now=$gz_webname;
		$sitemaplist=array();
		...
		$lang=$lang_now;
		$gz_weburl=$gz_weburl_now;
		$gz_webname=$gz_webname_now;
	}else{
		$sitemaplist=sitemaplist();
	}
	$gz_sitemap_max=50000;
	/*htmlÍøÕ¾µØÍ¼*/
	if($gz_sitemap_html){
		...
		$config_save.="<title>{$gz_title}</title>\n";
		$config_save.="</head>\n";
		$config_save.="<body>\n";
		$config_save.="<ul>\n";
		$i=0;
